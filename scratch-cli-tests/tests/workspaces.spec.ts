...
describe('Workspaces', () => {
  ...
  describe('show', () => {
    it('should retrieve a workspace by ID', () => {
      ...
      const shown = cli.json<{ id: string; name: string; version: number }>(['workspaces', 'show', workspaceId]);
  ...
  describe('list', () => {
    it('should include the created workspace in the list', () => {
      ...
      const list = cli.json<{ workbooks: Array<{ id: string; name: string }> }>(['workspaces', 'list']);
