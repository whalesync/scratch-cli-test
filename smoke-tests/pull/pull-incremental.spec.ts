...
describe.each(fixtures)('Pull incremental: $displayName', (fixture) => {
  ...
  it('detects new and removed records on re-pull', async () => {
    ...
    const job1 = await pullAndWait(api, workspace.workbookId, [workspace.dataFolderId]);
    expect(job1.state).toBe('completed');

    const files1 = await api.get(`/workbooks/${workspace.workbookId}/files/list/by-folder`, {
      folderId: workspace.dataFolderId,
    });
    const fileCount1 = files1.data.items.filter((item: any) => item.type === 'file' && item.name !== '.schema.json').length;
    ...
    // Re-pull
    const job2 = await pullAndWait(api, workspace.workbookId, [workspace.dataFolderId]);
    expect(job2.state).toBe('completed');

    // Verify updated file count
    const files2 = await api.get(`/workbooks/${workspace.workbookId}/files/list/by-folder`, {
      folderId: workspace.dataFolderId,
    });
    const fileCount2 = files2.data.items.filter((item: any) => item.type === 'file' && item.name !== '.schema.json').length;
