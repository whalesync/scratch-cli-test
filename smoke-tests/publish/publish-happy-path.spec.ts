  ...
  describe("create, edit, delete, then publish", () => {
      ...
      const createRes = await api.post(
        `/workbooks/${workspace.workbookId}/files`,
        {
          ...
          content: JSON.stringify({
            fields: { Name: "New Record", Status: "Active", Count: 999 },
          }),
        },
      );
    ...
    it("publishes all changes and verifies remote state", async () => {
      ...
      const newRecord = remoteRecords.find(
        (r) => r.fields.Name === "New Record",
      );
      expect(newRecord).toBeDefined();
      expect(newRecord!.fields.Count).toBe(999);

      // Verify the edited record was updated
      const updatedRecord = remoteRecords.find(
        (r) => r.fields.Status === "Updated via smoke test",
      );
