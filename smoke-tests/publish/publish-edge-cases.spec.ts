  ...
  it('large batch: creates 25+ records (exercises batch splitting)', async () => {
    ...
    for (let i = 0; i < 25; i++) {
      const res = await api.post(`/workbooks/${workspace.workbookId}/files`, {
        ...
        content: JSON.stringify({ fields: { Name: `Batch Record ${i}`, Status: 'New', Count: i } }),
      });
    ...
    const publishResult = await runPublish(api, workspace.workbookId, pipelineId!, 120000);
    ...
    const batchRecords = remoteRecords.filter((r) => String(r.fields.Name).startsWith('Batch Record'));
