...
describe.each(fixtures)('Publish with references: $displayName', (fixture) => {
  ...
  it('resolves circular foreign key references via BACKFILL phase', async () => {
    ...
    const pulledFiles = filesRes.data.items.filter((item: any) => item.type === 'file' && item.name !== '.schema.json');
    ...
    const createAuthor = await api.post(`/workbooks/${workspace.workbookId}/files`, {
    ...
    const createBook = await api.post(`/workbooks/${workspace.workbookId}/files`, {
      name: 'new-book.json',
      parentFolderId: workspace.dataFolderId,
    ...
    const newBook = remoteRecords.find((r) => (r.fields as any).Title === 'New Book');
