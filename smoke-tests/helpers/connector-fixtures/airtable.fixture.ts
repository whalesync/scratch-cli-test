...
const FAKE_AIRTABLE_URL = process.env.FAKE_AIRTABLE_URL ?? 'http://localhost:4656';
...
export const airtableFixture: ConnectorFixture = {
  ...
  async seed(admin: FakeAdminClient, opts?: { recordCount?: number }): Promise<SeedResult> {
    ...
    await admin.setup({
      bases: [{ id: TEST_BASE_ID, name: 'Smoke Test Base', permissionLevel: 'create' }],
      tables: [
        {
          baseId: TEST_BASE_ID,
          tables: [
      ...
      records: [
        {
          baseId: TEST_BASE_ID,
          tableId: TABLE_A_ID,
          records: [{ fields: { Name: 'Author A' } }, { fields: { Name: 'Author B' } }],
        },
        {
          baseId: TEST_BASE_ID,
          tableId: TABLE_B_ID,
          records: [{ fields: { Title: 'Book 1' } }, { fields: { Title: 'Book 2' } }],
        },
      ],
    });
