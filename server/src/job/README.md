...
Not all connectors support cursor-based resume yet. See [Connector Resume Plan](/docs/connector-resume-plan.md) for status.
...
When a job stalls and is re-dispatched, resume depends on the connector persisting its pagination cursor via `connectorProgress`. Not all connectors implement this yet — some restart from the beginning after a stall. See [Connector Resume Plan](/docs/connector-resume-plan.md) for per-connector status.
...
| Connector resume plan             | [`docs/connector-resume-plan.md`](/docs/connector-resume-plan.md)         |
