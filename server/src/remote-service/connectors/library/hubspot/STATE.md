...
- **v1 scope / known gap (DEV-10902):** only the HubSpot-DEFINED **default** association is written (`createAssociation` uses `/associations/default/…`); association *labels* aren't round-tripped on write even though we pull and store them verbatim (v3 inline `?associations=` returns e.g. `{ id, type: "quote_to_contact_signer" }`, which is kept on disk). `syncAssociations` diffs by `id` only, so a labeled association is re-created as the plain default and its label is lost; a delete removes ALL labels between the two records (v4 `DELETE …/associations/{toType}/{toId}`). This is why e-sign quotes (which require a "Signer" label to publish) are blocked from the recall dance — see Quotes below.
...
- **E-sign quotes are blocked from the recall dance (DEV-10902).** HubSpot won't publish an `hs_esign_enabled` quote without a contact carrying the **"Signer" labeled association**, and `syncAssociations` can't yet round-trip that label (it diffs by `id` only and creates via the v4 `default` endpoint), so recalling/re-publishing an e-sign quote would strip its signers and leave a live quote stuck unpublished. `applyQuoteUpdateRecallingIfPublished` therefore refuses **before** recalling — whenever the publish would recall the quote, **actually create/delete** an association (a by-id diff against the live quote, mirroring `syncAssociations`, so a no-op resync isn't caught), or move it to a published status — with a clear `QUOTE_ESIGN_NOT_EDITABLE` error ("edit this quote directly in HubSpot"). A property-only edit that keeps a draft quote in DRAFT — including a full publish whose associations match remote — touches none of those and still proceeds. Full signer/labeled-association round-trip is tracked in DEV-10902.
