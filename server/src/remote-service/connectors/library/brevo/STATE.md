...
- **Live spec:** `server/test/integration/brevo-connector.spec.ts` — 📄 ✅.
- **Runs in CI pipeline:** ❌ — self-skips (`describeIfKey`) until its credential is wired as a GitLab CI/CD variable.
- **Credentials / env vars:** `BREVO_API_KEY` — local in `server/.env.integration` (template `.env.integration.example`); CI in GitLab → Settings → CI/CD → Variables.
