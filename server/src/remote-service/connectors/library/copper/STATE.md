...
> **Do not delete this file.** It is generated by the `/connector-build-execute` skill and is the resumable record of what has been tested. Future work to extend the connector relies on it.
...
## Custom fields (mixed) — verbatim array, per-field editable columns; all 10 writable types round-tripped ✅
...
**Shape (verbatim array + annotation — DEV-10637, `copper-custom-fields.ts`):** Copper returns values as the array `custom_fields: [{ custom_field_definition_id, value }]`, which we **store exactly as-is** (Connector Prime Directive — no array→object reshape on disk). The array property is annotated with `x-scratch-array-keyed-by: { keyField: 'custom_field_definition_id', valuePath: 'value', columns: [...] }`, so the generic engines expose each element as its own editable column via the filtered path `custom_fields.[custom_field_definition_id=<id>].value` (one per definition, gathered under the "Custom Fields" banner). Key = the definition id (rename-stable, collision-free, path-safe). On publish the array is sent verbatim; `computeChangedFields` diffs it element-wise so a one-field edit still sends only `[{custom_field_definition_id, value}]` (per-field, and Copper merges by id). The id→name/type legend lives at the **schema root** via `x-scratch-agent-instructions`. _(Was reshaped to `{cf_<id>: value}` until 2026-07-01; the reshape was removed as a Prime Directive violation — GoHighLevel/Affinity still pending the same migration.)_
...
- **Array elements as editable columns without reshaping (`x-scratch-array-keyed-by`).** Scratch column paths were plain object dot-paths (arrays leaves on read, clobbered on write, auto-columns object-only), so an array element couldn't be a column — historically forcing an array→object reshape. The path grammar now has a **filter segment** `[<field>=<value>]` that `getByPath`/`setByPath`/`build-column-definitions` (and the schema-aware `computeChangedFields`) understand, so `custom_fields` stays a **verbatim array** and each element is editable via `custom_fields.[custom_field_definition_id=<id>].value` (DEV-10637). GoHighLevel/Affinity await the same migration off their reshapes. (Primitive in `@spinner/shared-types` `keyed-array.ts`; confirmed in `scratch-desktop` getByPath/setByPath + `build-column-definitions.ts`.)
...
- ✅ **Schema `required` is pinned to `id` only (DEV-10453 schema-validation-noise sweep).** Copper returns blank fields as `null` (e.g. a sparse company has `address`/`assignee_id`/`details` = null), and the CLI's `enforce_schema` validator flags a `required` field whose value is null (`"field 'X' is required but missing or null"`). TypeBox's `Type.Object` marks every non-`Optional` property `required`, so verbatim records failed validation. Fix: `pinRequiredToIdOnly` in `copper-json-schema.ts` sets `required: ['id']` on every entity + reference table — `id` is the only field Copper always returns non-null (matches the Webflow/Moco/Pipedrive fixes in the same sweep). Verify any connector this way right after pulling: `scratchmd validation dry-run --folder "<conn>/<folder>" --file <rec>.json --validation '[{"validator":"enforce_schema"}]'` → `[]`.
...
- **Notes:** No live integration spec yet (has unit tests). Reviewed by `/connector-build-execute` (see **Last run**).
