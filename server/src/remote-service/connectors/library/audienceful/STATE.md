...
# Audienceful — Connector Test Coverage
...
- **Live spec:** none yet — 📄 ❌ (no `server/test/integration/audienceful-connector.spec.ts`).
- **Runs in CI pipeline:** ❌.
...
- **Notes:** No live integration spec yet. Fake available at `test-api-fakes/audienceful`; has unit tests.
