...
- **Live spec:** `server/test/integration/airtable-connector.spec.ts (+ `airtable-create-schema.spec.ts`)` — 📄 ✅.
- **Runs in CI pipeline:** ❌ — self-skips (`describeIfKey`) until its credential is wired as a GitLab CI/CD variable.
- **Credentials / env vars:** `AIRTABLE_API_KEY` (+ `AIRTABLE_TEST_BASE_ID`) — local in `server/.env.integration` (template `.env.integration.example`); CI in GitLab → Settings → CI/CD → Variables.
- **Capabilities covered:** schemas ✅ · pull ✅ · publish (CRUD) ✅ · error handling ✅.
- **State model:** Mostly state-agnostic: shape assertions + CRUD self-provisions records inside a pre-existing base (`AIRTABLE_TEST_BASE_ID`).
- **Notes:** `airtable-create-schema.spec.ts` creates tables and **cannot delete them (no delete-table API)** — it litters the scratch base; keep it off the per-merge path.
