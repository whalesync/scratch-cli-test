...
- **Symmetric links / reciprocal FKs (DEV-10753):** `multipleRecordLinks` fields surface `inverseFieldId` (from Airtable's `inverseLinkFieldId`) and `isSingleValued` (from `prefersSingleRecordLink`) on their `x-scratch-foreign-key` annotation, so the create-plan generator recognizes a reciprocal pair (Airtable auto-creates a mirror link on the other table) and suggests only ONE side when both tables are in the plan — the single-valued/normalized side for a 1→N, one arbitrary side for an N→N.
