...
**10,000-result per-query limit (DEV-11267):** a single `dataSources.query` — one (filter, sorts) combination — returns at most 10,000 results; at the limit pagination just stops (`has_more: false`) with `request_status.type === 'incomplete'`. The pull therefore sorts every query ascending by `created_time` and, on an incomplete query, rolls to a **new** query filtered to `created_time` `on_or_after` the last returned row's `created_time` (the boundary is checkpointed in pull progress as `createdOnOrAfter`, alongside a `sortedByCreatedTime` flag that invalidates pre-sort cursors). Notion offers no page-id sort/filter (ids are random UUIDs), so `created_time` is the only viable continuation cursor; it's minute-granular, so the inclusive boundary makes the rolled window re-return the boundary minute — the pull loop skips those via a bounded dedup set (ids delivered at the current latest `created_time`, reset when it advances), which also avoids re-fetching their `page_content`; pages re-delivered across a job restart are absorbed by the idempotent commit path instead. The resume boundary is the newest `created_time` seen anywhere in the window, not just in the batch that reported the limit, so a page-less final batch doesn't abandon a resumable pull. >10k pages created in the same minute fails explicitly (non-advancing boundary) rather than looping. The Search-based standalone-pages pull has the same limit but **no** filter to continue past it: it logs a warning when incomplete. See `notion-query-continuation.ts` + the pull loop in `notion-connector.ts`, and `notion-connector-query-limit.spec.ts`.

**Compound-filter nesting (affects both the 10k continuation and incremental pulls):** Notion allows compound filters "nested up to two levels deep" — verified live on `Notion-Version: 2026-03-11` that `{ and: [{ or: [A, B] }, tsFilter] }` returns 200 and a third level returns a 400 `validation_error`. `addRequiredMemberToNotionFilter` (`notion-incremental.ts`) is the single place that bolts an extra required condition onto a user filter within that budget: it appends to an existing top-level `and` (no new level) and otherwise AND-wraps. The **only** un-combinable shape is a top-level `or` that already contains a nested compound — there, an incremental pull demotes to a full scan and a pull that hits the 10k limit throws rather than silently truncating.
...
## Gotchas
- **`excludePageContent` / `childContentMaxDepth` apply on EVERY path that builds a ConnectorFile, not just the full-table pull (DEV-11258).** The folder's page-content settings used to reach only `pullRecordFiles`; `pullRecordFilesByIds` (targeted pulls) and the refetch `updateRecords` does after each write always walked the whole block tree, so a sync that only writes properties (Live Export) still paid for every page's body on each publish. Both methods now take the folder's `DataFolderOptions` as a trailing `options` argument (the `pull-files` job and the publish run pass `dataFolder.options`, mirroring how the pull job passes them to `pullRecordFiles`), and every path shares `attachPageContent`. Byte-equality of the post-update refetch with a fresh pull includes `page_content`: a folder that excludes it pulls without it, so the refetch leaves it out too. A caller that passes no options gets full page content. See `notion-connector-page-content-settings.spec.ts`.
