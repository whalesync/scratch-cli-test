...
- **Multi-value source → text field keeps only the first value; plan warns (DEV-10956).** Notion `rich_text`/`title` properties are a non-scalar object envelope, so when a multi-valued source (Postgres `text[]`, an Airtable multi-select) is Live-Exported into a created Notion text field, the sync's cardinality reshape take-firsts (only the first array element lands) rather than comma-joining — a Notion text field's `object` logical type fails the picker's `TEXT_LIKE_DESTINATION_LOGICAL_TYPES` check. To surface this data loss, `NOTION_SCHEMA_CREATION_CAPABILITIES` declares `keepsOnlyFirstValueWhenMultiValueMappedToTextField: true`; the create-plan generator then emits a `downgraded` note ("only the first value will sync … additional values are dropped") the Live Export "tables & fields" editor renders as a per-field warning. Destinations whose text fields are scalar strings (Airtable, Postgres) comma-join losslessly and leave the flag false. See `schema-builder-plan-generator.ts` (`inferLogicalFieldType`) and `transform-picker.ts`.
