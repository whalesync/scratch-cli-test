...
| 4 | **All entities seeded & fetched** | ✅ | **20 tables** seeded + pulled into wkb `wkb_K7rI94Db0y` (core CRM + inventory + support + Stage History via UI + **Users**). The email-analytics family + Locking are now **filtered out** of `listTables` (`viewable: false`; unpollable — DEV-11111); Visits is a feature-gated table that only polls on SalesIQ-enabled orgs (runtime warn-and-skip pending); Functions polls fine (empty). |
...
Enumerated live from `/settings/modules` (26 modules total). `api_supported && !subform && viewable !== false` ⇒ candidate table. (The `viewable === false` guard was added for **DEV-11111** — see the exclusion note below.)
...
| Excluded UI/system modules: Home, Activities, Reports, Feeds, Documents, Emails, Email_Template__s, Email_Drafts__s | **weird** — `api_supported=false`; UI views/aggregates, not record collections | not exposed (filtered in `listTables` on `api_supported !== true`) | n/a — correctly excluded |
| Excluded reporting/aggregate modules: Email_Analytics, Email_Template_Analytics, Email_Sentiment, Locking_Information__s | **weird** — advertise `api_supported: true` but the records endpoint rejects every fetch with `NOT_SUPPORTED` "the given module is not supported in api". Empirically these are exactly the modules Zoho marks `viewable: false` (verified by hitting `GET /crm/v8/{module}` on all 50 modules of a live org — DEV-11111); every genuine record module, including read-only ones (Stage History, My Jobs), is `viewable: true`. | **filtered out** in `listTables` on `viewable === false` (explicit `=== false` so an absent flag never drops a real table) | n/a — correctly excluded (was DEV-11111) |
| Feature-gated module: Visits (SalesIQ) | **weird** — `api_supported: true`, `viewable: true`, metadata identical to a normal record module, but the records endpoint returns `403 FEATURE_NOT_ENABLED` "visits integration is not enabled" on orgs without the SalesIQ visits integration. **No metadata flag distinguishes it** — it polls fine on orgs that have the feature. | **NOT** filtered (would nuke a real table on orgs that have it). Needs runtime warn-and-skip on the pull path — tracked as the graceful-skip follow-up to DEV-11111. | ⬜ pending (pull-path skip) |
