...
const mockFindViewUniquelyAddressableColumnCandidates = jest.fn();
...
const mockFindViewsWithUniquelyAddressableColumns = jest.fn();
const mockFindViewRelationOid = jest.fn();
const mockFindRelationOidAndKind = jest.fn();
...
jest.mock('../../pg-common/knex-pg-client', () => {
  return {
    KnexPGClient: jest.fn().mockImplementation(() => ({
      ...
      findViewUniquelyAddressableColumnCandidates: mockFindViewUniquelyAddressableColumnCandidates,
      ...
      findViewsWithUniquelyAddressableColumns: mockFindViewsWithUniquelyAddressableColumns,
      findViewRelationOid: mockFindViewRelationOid,
      findRelationOidAndKind: mockFindRelationOidAndKind,
...
const BASE_TABLE_RELATION_OID = 25892;
const VIEW_RELATION_OID = 4242;
...
describe('SupabaseConnector.fetchJsonTableSpec', () => {
  ...
  beforeEach(() => {
    ...
    // Empty index-backed uniques trigger the view fallback, which first checks
    // whether the relation is a view at all; a base table (what most tests
    // model) resolves to relkind 'r' and skips the candidates lookup.
    mockFindRelationOidAndKind.mockResolvedValue({ oid: BASE_TABLE_RELATION_OID, relkind: 'r' });
    mockFindViewUniquelyAddressableColumnCandidates.mockResolvedValue([]);
  });
  ...
  it('uses a view-backed unique column as idPath and marks it read-only (DEV-11136)', async () => {
    // A view: no PK candidates, no index-backed unique columns, but its `id`
    // output column is backed by the base table's primary key.
    mockFindPrimaryColumnCandidates.mockResolvedValue([]);
    mockFindSingleColumnUniqueIndexColumns.mockResolvedValue([]);
    mockFindRelationOidAndKind.mockResolvedValue({ oid: VIEW_RELATION_OID, relkind: 'v' });
    mockFindViewUniquelyAddressableColumnCandidates.mockResolvedValue([
      { column_name: 'id', native_type: 'integer', is_primary_key: true },
    ]);
    const spec = await fetchSchema([
      buildColumn({ column_name: 'id', data_type: 'integer', udt_name: 'int4', is_nullable: 'YES' }),
      buildColumn({ column_name: 'name', is_nullable: 'YES' }),
    ]);

    expect(spec.idPath).toBe('id');
    const properties = spec.schema.properties as Record<string, Record<string | symbol, unknown>>;
    // The addressing column is read-only (it has no generated default on the
    // view for the isGeneratedColumn heuristic to catch)…
    expect(properties.id[X_SCRATCH_READONLY]).toBe(true);
    // …but the rest of an updatable view stays editable.
    expect(properties.name[X_SCRATCH_READONLY]).toBeUndefined();
  });

  it('marks every column of a non-updatable view read-only via is_updatable = NO', async () => {
    mockFindPrimaryColumnCandidates.mockResolvedValue([]);
    mockFindRelationOidAndKind.mockResolvedValue({ oid: VIEW_RELATION_OID, relkind: 'v' });
    mockFindViewUniquelyAddressableColumnCandidates.mockResolvedValue([
      { column_name: 'id', native_type: 'integer', is_primary_key: true },
    ...
    const spec = await fetchSchema([
      ...
      buildColumn({ column_name: 'author_name', is_nullable: 'YES', is_updatable: 'NO' }),
    ]);

    const properties = spec.schema.properties as Record<string, Record<string | symbol, unknown>>;
    expect(properties.id[X_SCRATCH_READONLY]).toBe(true);
    expect(properties.author_name[X_SCRATCH_READONLY]).toBe(true);
  });

  it('prefers an index-backed unique column over the view-backed fallback (and skips the candidates lookup)', async () => {
    mockFindPrimaryColumnCandidates.mockResolvedValue(['id']);
    mockFindSingleColumnUniqueIndexColumns.mockResolvedValue([
      { column_name: 'id', native_type: 'integer', is_primary_key: true },
    ]);
    ...
    expect(spec.idPath).toBe('id');
    expect(mockFindViewUniquelyAddressableColumnCandidates).not.toHaveBeenCalled();
  });

  it('marks the spec idPathRequiresUserSelection for a view with no unique-backed column (DEV-11136)', async () => {
    mockFindPrimaryColumnCandidates.mockResolvedValue([]);
    mockFindRelationOidAndKind.mockResolvedValue({ oid: VIEW_RELATION_OID, relkind: 'v' });
    mockFindViewUniquelyAddressableColumnCandidates.mockResolvedValue([]);
    const spec = await fetchSchema([
      buildColumn({ column_name: 'author_name', is_nullable: 'YES' }),
      buildColumn({ column_name: 'types_count', data_type: 'bigint', udt_name: 'int8', is_nullable: 'YES' }),
    ]);

    expect(spec.idPathRequiresUserSelection).toBe(true);
    // idPath falls back to the legacy 'id' collapse — consumers must gate on the marker.
    expect(spec.idPath).toBe('id');
  });

  it('leaves idPathRequiresUserSelection unset for a base table without unique columns', async () => {
    mockFindPrimaryColumnCandidates.mockResolvedValue([]);
    mockFindRelationOidAndKind.mockResolvedValue({ oid: BASE_TABLE_RELATION_OID, relkind: 'r' });
    const spec = await fetchSchema([buildColumn({ column_name: 'order_part', is_nullable: 'YES' })]);

    expect(spec.idPathRequiresUserSelection).toBeUndefined();
    expect(mockFindViewUniquelyAddressableColumnCandidates).not.toHaveBeenCalled();
  });

  it('deep-links remoteWebUrl to the dashboard table editor by relation OID', async () => {
    const spec = await fetchSchema([buildColumn({ column_name: 'id' })]);

    expect(spec.remoteWebUrl).toBe(
      `https://supabase.com/dashboard/project/${PROJECT_REF}/editor/${BASE_TABLE_RELATION_OID}?schema=public`,
    );
  });
  ...
  it('deep-links a view by its own OID too', async () => {
    ...
    const spec = await fetchSchema([buildColumn({ column_name: 'id', is_nullable: 'YES' })]);

    expect(spec.remoteWebUrl).toBe(
      `https://supabase.com/dashboard/project/${PROJECT_REF}/editor/${VIEW_RELATION_OID}?schema=public`,
    );
  });

  it('percent-encodes a schema name in the deep link', async () => {
    const connector = new SupabaseConnector({ connectionString: CONNECTION_STRING });
    mockFindAllColumnsInTable.mockResolvedValue([buildColumn({ column_name: 'id' })]);
    const spec = await connector.fetchJsonTableSpec({
      wsId: 'records',
      remoteId: [PROJECT_REF, 'my schema', 'records'],
    });

    expect(spec.remoteWebUrl).toContain('?schema=my%20schema');
  });

  it('omits remoteWebUrl when the relation is gone from the catalog', async () => {
    mockFindRelationOidAndKind.mockResolvedValue(null);
    const spec = await fetchSchema([buildColumn({ column_name: 'id' })]);

    expect(spec.remoteWebUrl).toBeUndefined();
  });

  it('omits remoteWebUrl for a non-Supabase-cloud host, which has no dashboard', async () => {
    // A self-hosted Supabase (or plain Postgres) reached through this connector:
    // the extracted "ref" means nothing on supabase.com, so emit no link at all.
...
describe('SupabaseConnector.listTables view handling (DEV-11136)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockDispose.mockResolvedValue(undefined);
    mockFindTablesWithUniqueColumns.mockResolvedValue(new Set(['public.authors']));
    mockFindTablesWithAutoGeneratedPK.mockResolvedValue(new Set(['public.authors']));
    mockFindViewsWithUniquelyAddressableColumns.mockResolvedValue(new Set(['public.authors_simple_view']));
    mockFindAllTablesExcludingSchemas.mockResolvedValue([
      { table_schema: 'public', table_name: 'authors', table_type: 'BASE TABLE' },
      { table_schema: 'public', table_name: 'authors_simple_view', table_type: 'VIEW' },
      { table_schema: 'public', table_name: 'author_stats_view', table_type: 'VIEW' },
    ]);
  });

  async function listTables() {
    const connector = new SupabaseConnector({ connectionString: CONNECTION_STRING });
    const previews = await connector.listTables();
    return new Map(previews.map((preview) => [preview.displayName, preview]));
  }

  it('enables a view whose output column is backed by a unique base column, with creates disabled', async () => {
    const previews = await listTables();
    const simpleView = previews.get('authors_simple_view');
    expect(simpleView?.disabled).toBeUndefined();
    expect(simpleView?.disabledCreates).toBe(true);
    expect(simpleView?.disabledReason).toBe('This is a view — new rows can only be created in its underlying tables');
    expect(simpleView?.metadata?.description).toBe('View "authors_simple_view" in schema "public"');
  });

  it('flags a view with no uniquely-addressable column as requiring an ID field selection (not disabled)', async () => {
    const previews = await listTables();
    const statsView = previews.get('author_stats_view');
    expect(statsView?.disabled).toBeUndefined();
    expect(statsView?.requiresIdFieldSelection).toBe(true);
    expect(statsView?.disabledCreates).toBe(true);
    expect(statsView?.infoNote).toContain('an ID column must be chosen');
  });

  it('leaves base-table handling untouched — an indexed table with an auto-gen PK is fully enabled', async () => {
    const previews = await listTables();
    const authors = previews.get('authors');
    expect(authors?.disabled).toBeUndefined();
    expect(authors?.disabledCreates).toBeUndefined();
    expect(authors?.metadata?.description).toBe('Table "authors" in schema "public"');
  });

  it('enables a materialized view with a single-column unique index, creates disabled with a matview reason', async () => {
    mockFindAllTablesExcludingSchemas.mockResolvedValue([
      { table_schema: 'public', table_name: 'authors_matview', table_type: 'MATERIALIZED VIEW' },
    ]);
    mockFindTablesWithUniqueColumns.mockResolvedValue(new Set(['public.authors_matview']));
    const previews = await listTables();
    const matview = previews.get('authors_matview');
    expect(matview?.disabled).toBeUndefined();
    expect(matview?.disabledCreates).toBe(true);
    expect(matview?.disabledReason).toBe(
      'This is a materialized view — its rows come from refreshing its query, so creates are not supported',
    );
    expect(matview?.metadata?.description).toBe('Materialized view "authors_matview" in schema "public"');
  });

  it('disables a materialized view without a unique index, with a matview-specific reason', async () => {
    mockFindAllTablesExcludingSchemas.mockResolvedValue([
      { table_schema: 'public', table_name: 'types_matview', table_type: 'MATERIALIZED VIEW' },
    ]);
    const previews = await listTables();
    const matview = previews.get('types_matview');
    expect(matview?.disabled).toBe(true);
    expect(matview?.disabledReason).toBe(
...
describe('SupabaseConnector pull duplicate-id guard for views (DEV-11136)', () => {
  ...
  async function pullRows(rows: Record<string, unknown>[][]) {
    let call = 0;
    mockSelectAll.mockImplementation(() => Promise.resolve(rows[call++] ?? []));
    const pulled: Record<string, unknown>[] = [];
    await connector.pullRecordFiles(
      buildIncrementalTableSpec(),
      ({ files }) => {
        pulled.push(...files);
  ...
  it('fails a view pull loudly when the id column repeats, even across batches', async () => {
    mockFindViewRelationOid.mockResolvedValue(4242);
    // A full first batch (READ_BATCH_SIZE = 500) keeps the pull loop fetching;
    // the duplicate id straddles the batch boundary.
    const fullFirstBatch = Array.from({ length: 500 }, (_, i) => ({ id: i }));
    const secondBatchRepeatingLastId = [{ id: 499 }, { id: 500 }];
    await expect(pullRows([fullFirstBatch, secondBatchRepeatingLastId])).rejects.toThrow(
      'returned multiple rows with the same "id" value (499)',
    );
  });

  it('passes a view pull whose id values are all distinct', async () => {
    mockFindViewRelationOid.mockResolvedValue(4242);
    const pulled = await pullRows([[{ id: 1 }, { id: 2 }, { id: 3 }]]);
    expect(pulled).toHaveLength(3);
  });

  it('skips the guard for base tables — index-backed uniqueness needs no runtime check', async () => {
    mockFindViewRelationOid.mockResolvedValue(null);
    const pulled = await pullRows([[{ id: 7 }, { id: 7 }]]);
...
describe('SupabaseConnector incremental pulls', () => {
  ...
  beforeEach(() => {
    ...
    mockFindViewRelationOid.mockResolvedValue(null); // default: pulling a base table, not a view
    connector = new SupabaseConnector({ connectionString: CONNECTION_STRING });
  });
