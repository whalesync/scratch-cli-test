...
# Generic API — Connector Test Coverage
...
- **Live spec:** none yet — 📄 ❌ (no `server/test/integration/generic-api-connector.spec.ts`).
- **Runs in CI pipeline:** ❌.
...
- **Notes:** No live integration spec, and out of scope for one: the read-only schema-driven connector is exercised per external service by the `/test-generic-connector` skill (coverage docs under `generic-api/coverage/`), not by a fixed live suite.
