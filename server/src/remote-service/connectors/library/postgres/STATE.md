...
# Postgres — Connector Test Coverage
...
- **Live spec:** `server/test/integration/postgres-create-schema.spec.ts + postgres-incremental-pull.spec.ts` — 📄 ✅.
- **Runs in CI pipeline:** ❌ — self-skips (`describeIfKey`) until its credential is wired as a GitLab CI/CD variable. runs against the CI job’s own `postgres:16` sidecar, not a third-party service.
- **Credentials / env vars:** `DATABASE_URL` — local in `server/.env.integration` (template `.env.integration.example`); CI in GitLab → Settings → CI/CD → Variables.
- **Capabilities covered:** schemas ✅ · pull ✅ · publish (CRUD) ❌ · error handling ❌.
- **State model:** State-agnostic — creates and drops its own schema (`scratch_create_schema_it`).
- **Notes:** **Partial**: schema-creation + incremental pull only; no core CRUD connector spec. Not a live external service.
