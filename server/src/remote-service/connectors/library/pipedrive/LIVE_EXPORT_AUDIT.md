...
Workbook wkb_6Ox8WmsueV (source conn recreated per attempt; dest conn coa_mENRpRPSOt cloned from coa_S8WUAP6AI4) → spreadsheet `1tvYhawQ0ZH7-Bk7RF1L0imN7gpq8qv9062J6enIO-os`. Tables deals/persons/organizations/leads/notes with `--skip-fields url` (DEV-11030, unchanged).

- **First run: 253/253 published, `failedOperations: []`.** Dropped FKs: Deals→stages/pipelines only (correct — not in export; same as prior destinations).
- **Verified via Sheets API**: typed values throughout (booleans native `false`, numbers numeric, dates/datetimes as serials in DATE/DATE_TIME-formatted columns), unicode/zero-width deal titles intact, 2001-char longtext deal present.
- **Second run: 246 ops churn** — DEV-10556 family, same scale as this source's other destinations. Not re-filed.
- **CRUD/drift**: destination-side CRUD + drift mechanics were proven in the SANITY→GOOGLE_SHEETS pass this same round (same engine path); pipedrive-side CRUD was proven in this doc's original audit. Not repeated per-destination.
- **Finding `[core/schema-builder]` (new, shared layer): existing-table name collision pass is not scoped to the create parent.** Attempt 1 of this run timed out (local server wedge) after creating folders; the retry in the same workbook created a FRESH spreadsheet, but the plan generator compared new table names against `listTables()` of the whole dest connection (which included attempt 1's folders in a DIFFERENT spreadsheet) and renamed everything to `Deals 2`, `Persons 2`, … inside a spreadsheet that contains no `Deals`. For destinations whose tables live under many parents (Sheets spreadsheets), the collision check needs `remoteParentId` scoping — and the `scratch-new-spreadsheet` sentinel can never match a listed parent at all. Cosmetic here (suffix renames), but worth fixing alongside the sentinel-resume design question already documented in the sheets STATE.md.

Harness note: this round added `tools/live-export-audit/long-timeouts.mjs` (node --import preload raising undici's 300s fetch timeout — slow-source runs against a busy local server can exceed it).
