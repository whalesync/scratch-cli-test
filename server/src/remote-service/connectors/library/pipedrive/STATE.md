...
# Pipedrive — Connector Test Coverage
...
- [x] DEV-11051 pt1 [view]: activity `participants` (the "linked people", verbatim `[{person_id, primary}]`) surfaced as a multi-value FK to persons (flattened to its person ids via a display transformer + editable codec) instead of a raw JSON object — landed 2026-07-24
- [x] DEV-11051 pt2: Pipedrive OAuth disabled in every environment (public marketplace app not approved) — connector declares no `oauth` metadata and drops `'oauth'` from `supportedAuthMethods`; provider plumbing left wired for internal test-app use — landed 2026-07-24
- [x] DEV-11051 pt3 [transport]: Postgres/Supabase column names containing `" as "` (e.g. "Marked as done time") were split by knex's identifier formatter into `"Marked" as "done time"`, breaking CREATE/INSERT/SELECT/UPDATE — fixed at the KnexPGClient choke point (`applyVerbatimIdentifierQuoting` rejoins the mis-split identifier) — landed 2026-07-24
- [x] DEV-11043 [transport]: static-schema entities (leads/notes/pipelines/stages) `add_time`/`update_time`/`archive_time` lacked `format: 'date-time'`, so the default view typed them as text and they exported as text columns — annotated them via a `dateTimeOrNull()` helper matching the dynamic builder's `_time` convention — landed 2026-07-24
...
- **Live spec:** `server/test/integration/pipedrive-connector.spec.ts` — 📄 ✅.
- **Runs in CI pipeline:** ❌ — self-skips (`describeIfKey`) until its credential is wired as a GitLab CI/CD variable.
- **Credentials / env vars:** `PIPEDRIVE_API_KEY` — local in `server/.env.integration` (template `.env.integration.example`); CI in GitLab → Settings → CI/CD → Variables.
