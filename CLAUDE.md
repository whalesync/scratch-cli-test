...
- **GCP projects:** The Scratch infrastructure lives in two GCP projects — `spv1eu-test` (test) and `spv1eu-production` (production). Use these for `gcloud` commands such as Cloud Logging queries, e.g. `gcloud logging read '<filter>' --project=spv1eu-production`. (The `connect_to_gcp_db_readonly.sh` script derives the same names as `spv1eu-${env}`.)
