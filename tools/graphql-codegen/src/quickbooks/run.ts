 ...
 */
const SUPPLEMENTARY_FIELDS: Record<string, Record<string, JsonSchemaProperty>> = {
  Bill: {
    VendorAddr: { type: ['null', 'object'], properties: {} },
  },
  CompanyInfo: {
    CustomerCommunicationEmailAddr: { type: ['null', 'object'], properties: {} },
...
async function fetchManifest(): Promise<string> {
  ...
  const response = await axios.get<string>(MANIFEST_URL, { responseType: 'text' });
...
function extractStreams(manifest: unknown): AirbyteStream[] {
  ...
  const streamsObj = definitions?.['streams'] as Record<string, Record<string, unknown>> | undefined;
...
`;
  ...
  const registryEntries = entityNames.map((name) => `  ${name}: ${name}Schema,`).join('\n');
  const registry = `
/**
...
async function main(): Promise<void> {
  ...
  for (const [streamName, entityName] of Object.entries(ENTITY_MAP)) {
    ...
    const fieldCount = Object.keys(jsonSchema.properties).filter((k) => k !== 'airbyte_cursor').length;
    console.log(`  ${entityName.padEnd(20)} ${String(fieldCount).padStart(3)} fields`);
  }

  // Add CompanyInfo (not in Airbyte manifest)
  schemas.push({ entityName: 'CompanyInfo', code: generateCompanyInfoSchema() });
  ...
  const output = generateOutputFile(schemas, entityNames, MANIFEST_URL, generatedAt);
  ...
  execSync(`"${prettierBin}" --write "${OUTPUT_FILE}"`, { cwd: repoRoot, stdio: 'inherit' });

  console.log(`\nDone! Run 'yarn build' from the repo root to verify.`);
}

main().catch((error) => {
  console.error('Codegen failed:', error instanceof Error ? error.message : error);
  process.exit(1);
});
