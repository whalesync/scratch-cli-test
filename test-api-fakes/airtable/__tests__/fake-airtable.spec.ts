...
describe("test admin", () => {
  it("POST /test/setup creates bases, tables, and records", async () => {
    ...
    const basesRes = await request(app)
      .get("/v0/meta/bases")
      .set(AUTH)
      .expect(200);
    ...
    const tablesRes = await request(app)
      .get("/v0/meta/bases/appABC/tables")
      .set(AUTH)
      .expect(200);
...
describe("GET /v0/meta/bases/:baseId/tables", () => {
  it("returns 404 for unknown base", async () => {
    const res = await request(app)
      .get("/v0/meta/bases/appNONE/tables")
      .set(AUTH)
      .expect(404);
...
describe("error simulation", () => {
  it("returns 429 with Retry-After when rate limit is simulated", async () => {
    await request(app)
      ...
      .send({ count: 2, retryAfterSeconds: 5 })
      .expect(200);
  ...
  it("does not apply error simulation to /test/ endpoints", async () => {
    await request(app)
      .post("/test/simulate-rate-limit")
      .send({ count: 100, retryAfterSeconds: 1 })
      .expect(200);
