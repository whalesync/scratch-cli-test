 ...
 *     "What was about to be published when the plan ran."
  ...
  const { data, error, isLoading } = useSWR<{ original: string | null; modified: string | null }, Error>(
    ...
    async () => {
      ...
      const [originalRes, modifiedRes] = await Promise.allSettled([
        publishApi.getRepoFile(workbookId!, filePath!, originalRef, connectorAccountId!),
        publishApi.getRepoFile(workbookId!, filePath!, modifiedRef, connectorAccountId!),
      ]);
      return {
