...
- **Webflow** — the REST API v2 **does** expose a `lastUpdated` filter on the List Collection Items endpoint (added in the 08082025 "Enhanced filtering and sorting" changelog: `lastUpdated` + `createdOn`, both `lte`/`gte`, plus sort), but the **installed `webflow-api@3.2.1` SDK does not surface it** — `ItemsListItemsRequest` exposes only `cmsLocaleId`, `offset`, `limit`, `name`, `slug`, `lastPublished`, `sortBy` (`lastPublished|name|slug`), and `sortOrder`. There is no escape hatch in 3.2.1: the compiled `__listItems` destructures a fixed key set (an extra `lastUpdated` key passed via a cast is silently dropped), and `RequestOptions` has no query-param passthrough. The only ways forward are (a) bump the SDK (≥3.3.4, which very likely exposes the filter — confirm) or (b) issue a raw v2 REST request that bypasses the typed method and its `TooManyRequestsError`-based rate-limit retry. Both are out of scope here, so Webflow stays full-scan until the SDK is bumped. *(This reverses the earlier "Feasible" verdict — the param exists at the API layer, but not through the layer the connector actually calls.)*
