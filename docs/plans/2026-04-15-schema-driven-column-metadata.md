...
Whatever save path exists today — whether it eventually honors the plan in [docs/plans/2026-04-09-desktop-schema-driven-field-save.md](docs/plans/2026-04-09-desktop-schema-driven-field-save.md) or not — should continue to work because `NormalizedRecordRow.raw` preserves the original JSON values. The stringified display values are derived-only; they are never written back to disk. Tests in §10 cover this (round-tripping the raw JSON through `projectRecordToNormalizedRow` must not mutate it).
...
- Related save-path plan: [docs/plans/2026-04-09-desktop-schema-driven-field-save.md](docs/plans/2026-04-09-desktop-schema-driven-field-save.md)
