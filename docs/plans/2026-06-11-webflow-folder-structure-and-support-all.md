...
- [~] **T2 (P1, human: ~3d / CC: ~1h)** — code-migrations — the folder-move migration (move → flip). 🟨 **Core done** (uncommitted, branch `webflow-support-all-v1`). **Quiesce (T4) + integration/E2E deferred.**
  ...
  - **Deferred:** the per-connection **quiesce wrapper (T4)** — cancel non-terminal PublishPlans, drain/dequeue jobs, block live edits, persist+restore schedule state; and **integration/E2E** coverage of the raw-SQL atomic executor (a Jest-mocked `$transaction` can't exercise the boundary-prefix SQL). Until T4, run `webflow-folder-restructure` only on an idle connection.
