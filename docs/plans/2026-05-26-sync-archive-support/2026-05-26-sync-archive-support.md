...
- **T9 (Lane C) — Phase 3 backfill descriptor** (`server/src/code-migrations/sync-mapping-v2-backfill.ts`, `code-migrations.controller.ts`, `code-migrations.module.ts`, `server/src/metrics/custom-metrics.ts`)
  - **Pure DI'd core** (`sync-mapping-v2-backfill.ts`, mirrors `notion-data-source-backfill.ts`): `backfillSyncMappingRow(row, deps)` parses the frozen v1 `mappings` via `syncMappingV1Schema`, runs `transformV1ToV2`, and (unless `dryRun`) writes `mappingsV2` through an injected compare-and-set. Returns `'transformed' | 'skipped_concurrent_write' | 'errored'`. Malformed v1 on disk is caught and returned as `errored` (accumulated in the batch summary, never thrown). `emptySyncMappingV2BackfillSummary` / `accumulateSyncMappingV2Backfill` tally the run and capture errored `{ syncId, message }`. `SYNC_MAPPING_V2_BACKFILL_AUDIT_MARKER` tags audit entries for rollback discoverability.
  - **Controller orchestrator** (`runSyncMappingV2Backfill` + `buildSyncMappingV2BackfillDeps`): registered as the `sync-mapping-v2-backfill` descriptor in `AVAILABLE_MIGRATIONS` + the `runMigration` switch — same admin path (`POST /code-migrations/run`, `ScratchAuthGuard` + `hasAdminToolsPermission`) as the existing migrations. Candidate query selects `Sync` rows where `mappingsV2 IS NULL` (`{ equals: Prisma.DbNull }`), targeted by `ids` or oldest-first up to `qty`. The compare-and-set is `updateMany({ where: { id, updatedAt: previouslyRead, mappingsV2: { equals: Prisma.DbNull } }, data: { mappingsV2 } })` — a concurrent `updateSync` (which sets `mappingsV2` and bumps `updatedAt`) or a parallel batch makes it a 0-row no-op. One `AuditLogService` entry per migrated row (skipped for workbook-less syncs). Returns the in-repo `MigrationResult` shape (`{ migratedIds, remainingCount, migrationName }`).
  - **Idempotent + non-destructive + parallel-safe**: the `mappingsV2 IS NULL` filter + compare-and-set mean re-runs and concurrent batches skip already-migrated rows; the frozen v1 `mappings` column is never modified, so the migration reverses by clearing `mappingsV2`.
  - **Metrics** (`custom-metrics.ts`): `BACKFILL_SYNC_MAPPING_V2_TRANSFORMED_TOTAL` (EVENT_COUNT, per-batch transform tally) and `BACKFILL_SYNC_MAPPING_V1_REMAINING` (INSTANTANEOUS_COUNT gauge of syncs still on v1 — the explicit Phase 4 drop trigger). Both `NO_DIMENSION`, emitted once per run. `MetricsModule` wired into `CodeMigrationsModule`; controller injects `CustomMetricsService`.
  - **ESLint choke point**: the candidate `findMany` carries one justified `// eslint-disable-next-line no-restricted-syntax` (it deliberately reads the raw pre-v2 `mappings` column, the one read that must bypass `parseStoredMappings`); `count` is not a `find*`, so it needs no disable.
  - **Plan reconciliation**: the plan body sketched a cursor-based descriptor returning `{ processed, updated, lastId, isDone }`. The in-repo `code-migrations` pattern (per R1: "use the in-repo pattern, don't build a parallel controller") uses `RunMigrationDto { qty, ids }` → `MigrationResult { migratedIds, remainingCount, migrationName }`. Idempotency comes from the `mappingsV2 IS NULL` filter (the implicit cursor), exactly like the notion backfill's `tableId.length < 2` filter — so the in-repo shape carries the plan's intent without a parallel cursor protocol.
  - **Deferred from the plan's Phase 3 (with rationale)**: the **local-dev auto-run on server start** (gated by `SCRATCH_DEV_AUTO_MIGRATE_SYNC_MAPPING_V2`) and the **kill-switch endpoint** are NOT implemented. There is zero in-repo precedent for `onApplicationBootstrap`/`SCRATCH_DEV_*` hooks, and the production sweep is manual per-batch (`POST /code-migrations/run`), so there is no internal multi-batch loop for a kill-switch to stop — ops simply stops invoking. Local devs run the admin endpoint once. Filed as follow-ups; revisit if an automated prod sweep loop is later built.
  - **Per-row error isolation** (added after adversarial review): the orchestrator loop wraps each `backfillSyncMappingRow` call in try/catch — a transient CAS throw (deadlock, dropped connection) is isolated to that row (recorded as `errored`, retried next sweep) rather than aborting the batch. The `logAudit` dep impl catches + warn-logs audit-write failures (best-effort; the row is already migrated), matching the T14 sync-run audit convention. A code comment documents the qty-mode head-of-line behavior (a persistently-malformed oldest prefix can stall progress and hold `remainingCount > 0`; drain/repair via ids-mode — the `summary.errors` list names the stuck ids).
  - **Tests**: `__tests__/sync-mapping-v2-backfill.spec.ts` (7 core tests: transform+write+audit, concurrent-write skip, malformed-v1 errored, dry-run, workbook-less audit-skip, summary accumulation). `code-migrations.controller.spec.ts` extended (descriptor presence + qty-mode/ids-mode/concurrent-skip orchestration asserting the CAS `where` shape and metric emission, plus CAS-throw isolation and audit-failure-still-migrates).

- **T10 (Lane C) — Phase 2 v1-write rejection guard** (`server/src/sync/sync.service.ts`)
  - In `updateSync`, the existence read now selects `{ id, mappingsV2 }`. When `mappingsV2 !== null` (the row has crossed to v2) AND the incoming `body.mappings.version === 1` (a stale v1-only client), the save is rejected with `ConflictException` (HTTP 409, body `{ error: 'SYNC_MAPPING_V1_WRITE_REJECTED', message, syncId }` — matching the `cli/upload-patch.controller.ts` structured-409 convention). This closes the D6-storage silent-shadow gap: a v1 client would otherwise write to the frozen `mappings` column and have its edits shadowed by the authoritative `mappingsV2`. The guard fires before any write path. `createSync` needs no guard (always a fresh row). A `TODO(DEV-10008)` marks it for Phase 4 removal.
  - **Tests**: three new `updateSync` specs (reject v1 body on migrated row; allow v2 body on migrated row; allow v1 body on not-yet-migrated row). `MOCK_SYNC` gained `mappingsV2: null` to mirror what Prisma returns for an un-migrated row.

- **Verified (T9 + T10):** root `yarn build` clean across all 14 packages; `server/` `yarn lint` (`--max-warnings=0`) clean; `yarn jest src/code-migrations src/sync/__tests__/sync.service.spec.ts` → 87 tests pass. A 3-lens adversarial review (concurrency/migration, T10 guard, Prisma/types) returned no blockers; its two `minor` findings (batch error-isolation, qty-mode stall doc) were addressed above. Integration tests not run (require local DB); end-to-end backfill against real rows is deferred to Lane E. Note: a raw `tsc --noEmit` from `server/` surfaces ~250 pre-existing type errors, all in spec/test files (the real `nest build` excludes `**/*spec.ts` via `tsconfig.build.json`, so `tsc --noEmit` is not a project gate). One of those — `sync.service.spec.ts`'s `LookupTools` mock missing `matchDestinationAssetByHash` (introduced by the merged T21 commit `cbcc5536`) — was in a file this MR already edits, so it was fixed here; that spec is now `tsc`-clean. The remaining test-file errors are unrelated to this work and left as a separate cleanup (no gate enforces them today).

- **T15 / T16 / T20 (Lane E) — Test coverage for the v2 executor, Pass 3, the backfill, and the v1-compat CI gate**
  - **`server/test/integration/sync-mapping-v1-compat.spec.ts`** (4 tests) — the focused regression gate the new `sync_v1_compat` CI job runs in isolation. Proves existing v1 `Sync.mappings` run byte-for-byte identically through the v2-internal executor: unchanged create + merge behavior, Pass 3 is a no-op for v1 syncs (unmatched dest records untouched, counts zero), and a v1 mapping produces byte-identical writes to its `transformV1ToV2` equivalent (update-only fixture so there are no random temp IDs to diverge).
  - **`server/test/integration/sync-archive-e2e.spec.ts`** (8 tests) — end-to-end Pass 3 against a real Postgres DB with mocked git/data-folder I/O (mirrors `sync-service.spec.ts`). Covers the DEV-10008 worked example (archive unmatched-with-key, leave hand-authored content alone, matched records get the matched-bucket constant), `withoutMatchKey: 'apply'` (sync owns the whole collection), and every Pass 3 gate: policy all-ignore, `onlySourceFilePath` (syncOneRecord) scope, `recordMatching` unset, match-key column missing from the destination schema (skips, never crashes), the defensive constant-on-match-key strip, and whitespace-match-key classification.
  - **`server/test/integration/sync-mapping-v2-backfill-db.spec.ts`** (6 tests) — DB-backed round-trip for the T9 backfill. Runs the real `backfillSyncMappingRow` against a real `Sync` row with the exact compare-and-set the controller injects: transform v1 → `mappingsV2` while freezing the v1 `mappings` column, one audit entry tagged with `SYNC_MAPPING_V2_BACKFILL_AUDIT_MARKER` when an org is known, skip-already-v2, CAS conflict on a concurrent `updatedAt` bump, malformed-v1 → `errored` (no throw, no mutation), and idempotent re-run.
  - **`server/src/sync/__tests__/sync-mapping.schema.spec.ts`** (15 tests, unit) — fills the save-time validation gap: the v2 zod refinements were previously untested. Covers refinement (a) column-source-not-on-`unmatched`/`always`, (b) constant-can't-target-the-match-key-column, (c) JSON-primitive-only constants, (d) duplicate `(destinationColumnId, when)`, the `source.kind` discriminated union, transformer/transformers exclusivity, policy enums, and the `version` literal — plus the legal multi-rule-same-column case and v1 acceptance.
  - **T20 — `sync_v1_compat` CI gate** (`gitlab-ci/stages/01-build-and-test.yml`). New job extending `.integration-test-server` (reuses the postgres service + `yarn migrate`) but scoping jest to the v1-compat spec only (`yarn run test:integration sync-mapping-v1-compat`). Documented inline as the green-light precondition for the v1 → v2 rollout. Same `rules` as `integration test server` (server changes / merge to master).
  - **Reconciliation against the stale eng-review test plan (2026-05-27, predates the migration redesign):** that plan referenced `WRITE_SYNC_MAPPING_V2`, a `/sync-mapping-v2/stop` kill-switch, a cursor/`lastId` sweep, and "orphan" terminology — all superseded. Tests were written against the shipped design: no writer flag, backfill via the `code-migrations` `sync-mapping-v2-backfill` descriptor + real CAS, and `unmatchedWithMatchKey` / `unmatchedWithoutMatchKey` vocabulary.
  - **Verified:** root `yarn build` clean across all 14 packages; `server/` `yarn typecheck` (`tsc --noEmit`, the DEV-10258 CI gate) and `yarn lint-strict` (`--max-warnings=0`) clean; the three new integration specs run green together against the local DB (18 tests); `yarn jest src/sync src/code-migrations` → 574 unit tests pass (incl. the 15 new schema-validation tests).

- **T22 + T17/T23 (Lane F) — `SyncExceptionFilter` + constant-type validation + `server/src/sync/README.md` refresh**
  - **`SyncExceptionFilter`** (`server/src/exception-filters/sync.exception-filter.ts`, registered in `main.ts` `useGlobalFilters`) maps the typed sync-mapping errors to stable, machine-readable HTTP responses: `SyncMappingNormalizeError` → 500 `SYNC_MAPPING_NORMALIZE_FAILED`, `SyncMappingVersionError` → 500 `SYNC_MAPPING_UNKNOWN_VERSION`, `ConstantTypeMismatchError` → 400 `INVALID_CONSTANT_TYPE`. `syncId` is read from the route param (`request.params.syncId`) when present (null on create — correct, not an error). It is reachable today: `transformV1ToV2` throws the two normalize/version errors on the HTTP `sync-one-record` / save paths.
  - **Convention deviation (intentional):** the eng review said "extending `BaseExceptionFilter`," but the two existing filters (`connector.exception-filter.ts`, `generic-errors.exception-filter.ts`) implement `ExceptionFilter` directly and write the response via `host.switchToHttp().getResponse().status().json()`. `SyncExceptionFilter` follows the in-repo convention — it only `@Catch`es its own error classes, so there is no unhandled case to delegate to a base filter, and direct instantiation in `useGlobalFilters` avoids the `BaseExceptionFilter` `applicationRef` footgun.
  - **`ConstantTypeMismatchError`** added to `packages/shared-types/src/sync-mapping.ts` (colocated with the other two sync-mapping errors) with `destinationColumnId` / `expected` / `got`. Its **thrower** was wired so the filter's 400 branch is real, not dead infra: a new pure `findConstantTypeMismatches(destSchema, columnMappings)` in `schema-validator.ts` compares each `{ kind: 'constant' }` value's type against its destination column type (numeric constants accepted by `number`/`integer`; `null` always allowed; untyped/`Type.Any()` columns skipped). `createSync`/`updateSync` throw the first mismatch inside the existing `validateMappings`-gated loop (gated like the sibling `validateSchemaMapping` check; needs the dest schema). Zero regression risk — constants are a v2-only feature with no production data yet.
  - **Reconciliation — `BackfillTransformError` / `UnsupportedWhenError` NOT created.** The eng review enumerated five exception classes, but these two were superseded by how the implementation actually shipped: T9's backfill accumulates malformed-v1 rows as an `errored` status in its batch `summary.errors` list (no thrown class), and T7's Pass 3 emits `WSLogger.warn` for an unsupported `when` / a constant targeting the match-key column (no thrown class). Creating them now would be unused dead code. The filter only `@Catch`es the three error classes that reach the HTTP layer.
  - **README refresh** (`server/src/sync/README.md`) covers both T17 and T23 (which overlap — both are the README): rewrote the Data Model section for the v1+v2 dual-column storage and the `getSync`/`getMappings` read choke point; restructured "Sync Execution Flow" into the Pass 1 / Pass 2 / Pass 3 model with an executor ASCII diagram; added an "Unmatched-record policies" section with the DEV-10008 archive worked example and the v2 validation invariants; updated Create/Update Sync (v1|v2 body, `mappingsV2` write, sentinel-v1, the 409 v1-write guard); documented the constant-type check + the typed-error→HTTP table in Schema Validation; refreshed Key Files (added `sync-execution.ts`, the filter). Corrected the stale "validation always runs" line to reflect the `validateMappings` gate.
  - **Tests**: `schema-validator.spec.ts` +11 `findConstantTypeMismatches` cases (string/number/boolean/integer compat, null skip, object-column mismatch, nullable unwrap, unknown/untyped-column skip, column-source ignored, multi-mismatch); `exception-filters/sync.exception-filter.spec.ts` (new, 4 tests — the three mappings + null `syncId` on create); `sync.service.spec.ts` +1 (createSync throws `ConstantTypeMismatchError`, with the schema-validator mock extended to expose `findConstantTypeMismatches`).
  - **Verified:** root `yarn build` (14 packages) clean; `server/` `yarn typecheck` + `yarn lint-strict` clean; root `yarn lint` (5 packages) clean; `prettier --check` clean on all changed files incl. the README; `yarn jest src/sync` → 547 pass + the new filter spec.

- **T11 + T12 (Lane D) — v2-aware sync editor + v1 JSON paste auto-upgrade** (`client/src/app/workbook/[id]/components/MainPane/SyncEditor.tsx`, new `sync-editor-model.ts`, `packages/shared-types/src/db/sync.ts`, `client/jest.config.ts`, new `client/__mocks__/nanoid.js`)
  - **Pure model extracted** to `client/.../MainPane/sync-editor-model.ts` (no React, unit-tested): the editor's in-memory `FolderPair`/`FieldMapping` types plus conversions `folderPairsToV2` / `v2ToFolderPairs` / `storedMappingToV2` / `resolveStoredMapping` / `coerceUnknownToV2` (accepts v1 **or** v2 paste, upgrades v1 via `transformV1ToV2`), and derived-state helpers `groupByDestColumn` / `isSimpleGroup` / `findConflictingMappingIds` / `validateV2Semantics` / `constantDefaultForType`. `FieldMapping` carries `when` + `kind: 'column' | 'constant'` (+ `sourceField`/`transformers` or `constantValue`); `FolderPair` carries the three policy choices. 23 colocated unit tests (`__tests__/sync-editor-model.test.ts`) incl. the DEV-10008 archive round-trip and the three semantic refinements.
  - **Load-path fix (the gate this lane closes).** `findAllForWorkbook`/`findOneForWorkbook` (`sync.service.ts:567,594`) return the **raw row** with both `mappings` (frozen v1/sentinel) and `mappingsV2`; the old editor read `existingSync.mappings.tableMappings` and would render **empty** for any post-T5 sync (sentinel `{version:1,tableMappings:[]}`). The editor now resolves `mappingsV2 ?? mappings`, normalizes v1→v2 in memory, then builds `FolderPair[]`. Added `mappingsV2: SyncMappingV2 | null` to the client-facing `Sync` entity (`packages/shared-types/src/db/sync.ts`) to mirror the Prisma column.
  - **Write path** emits **v2** (`folderPairsToV2`) via the existing `SaveSyncBody` (already `StoredSyncMapping`); the server normalizes + persists to `mappingsV2`. JSON-paste mode auto-upgrades v1 silently (toast: `Sync upgraded from v1 to v2 format`).
  - **Grouped stacked render (D3/T25/T32).** A plain matched column-copy renders as today's flat source→dest row (zero visual change for normal syncs); a dest column with constants / an unmatched-side rule / multiple rules renders as a stacked group — dest header, matched rule on top, unmatched rule(s) indented with an "on unmatched:" sub-label and a left accent. Per-rule **Source-column / Constant** picker (matched rows); type-driven constant editor (`Switch`/`NumberInput`/`TextInput` by dest column type) (T27). Preview pane only projects matched column copies to the v1 `previewRecord` endpoint.
  - **Advanced-policy accordion (UI-R1/T26)** at the form bottom, collapsed by default: source policy first (`SegmentedControl`), then the two destination-policy toggles with the locked help copy + visible labels + aria-labels. **`+ Add unmatched rule`** affordance is hidden unless a destination policy is `apply` (UI-R3/T29).
  - **ConfirmDialog (D2/T24)** on the first `ignore→apply` flip of a destination policy on an already-run sync (`existingSync.lastSyncTime`), once-per-flip per session. **Deviation:** the plan wanted a `{count}` of affected records from server-side `destinationRecordsByPath`, which the client doesn't have — the dialog drops the count and keeps the safety wording (no new endpoint, matching the plan's "no new server endpoint" constraint).
  - **Conflict rendering (T28)**: `findConflictingMappingIds` flags `(destinationColumnId, when)` collisions with a red left-border + `aria-describedby` helper text; `validateV2Semantics` mirrors the server's v2 zod refinements (column-only-matched, no constant on the match key, one rule per `(column, case)`) and gates save in **both** modes so an illegal shape gets a friendly inline message instead of a raw zod 400/500 (closes a review finding where a pasted illegal column+unmatched mapping silently corrupted).
  - **Component/copy deviations (minor, intentional):** `+ Add unmatched rule` uses a subtle `Button` (matches the sibling "Add column mapping") instead of `IconButtonGhost` (which is icon-only, `aspectRatio: 1`); conflict accent is 3px (matches the row's existing accent system) and uses `var(--mantine-color-red-6)` since `--fg-error`/`--fg-tertiary` don't exist in this codebase (the plan pre-authorized the `--fg-secondary`/red fallback).
  - **Test infra:** `client/jest.config.ts` maps `nanoid` (ESM-only, untransformed by `next/jest`) to a deterministic CJS mock (`client/__mocks__/nanoid.js`) so client tests can import `@spinner/shared-types` runtime values (its id helpers `require('nanoid')`) — previously only type-only imports worked.
  - **Verified:** root `yarn build` (14/14 turbo tasks incl. `client:build`) clean; client `tsc --noEmit` clean; client + `shared-types` `yarn lint` clean; `prettier --check` clean; `yarn jest` (client) → 75 pass (4 suites, incl. 23 new model tests). Two independent adversarial reviews run; all Critical/Major findings fixed.

### Next up

- **Phase 3 follow-ups deferred from T9** — local-dev auto-run on bootstrap + kill-switch endpoint (see T9 entry rationale).
- **Deferred validation note** — the constant-type check is gated behind `validateMappings` (like the sibling source→dest schema check). If the constant invariant should be enforced unconditionally at save time, that's a small follow-up.
- **Lane D follow-ups (optional polish):** surface refinement (a)/(b) inline (not just at save) as the user edits; the ConfirmDialog `{count}` if a cheap count source is later added; explicit-`null` constant affordance for nullable columns.
...
- **Lane C** — **complete** (T5, T9, T10). Backfill descriptor + v1-write rejection guard land the Phase 2/3 storage migration machinery; local-dev auto-run + kill-switch deferred as follow-ups.
- **Lane E** (T15, T16, T20) — **complete**. Three new integration specs (v1-compat regression gate, Pass 3 archive E2E, DB-backed backfill round-trip) + a v2-schema-validation unit spec + the `sync_v1_compat` CI gate. 18 integration + 15 unit tests, all green.
- **Lane D** (T11, T12) — **complete**. v2-aware sync editor: grouped/stacked rules, type-driven constant editor, advanced-policy accordion, ConfirmDialog on first apply, inline conflict + semantic validation, v1 JSON paste auto-upgrade, and the `mappingsV2 ?? mappings` load-path fix that lets the editor render v2 syncs (the gate for deploying the server changes). Design-review tasks T24–T32 implemented (with the minor documented copy/component deviations).
- **Lane F** — **complete**. T19 + T18 landed earlier; T22 (`SyncExceptionFilter` + `ConstantTypeMismatchError` + its save-time thrower) and T17/T23 (`server/src/sync/README.md` refresh) landed this MR. `BackfillTransformError` / `UnsupportedWhenError` intentionally not created — superseded by the shipped T9 errored-status accumulation and T7 WSLogger warnings.

**All implementation lanes (A–F) complete.** Remaining work is the Phase 2/3/4 rollout sequence (client-version soak → backfill sweep → v1 column drop) and the optional follow-ups above — not new feature code.
