...
# Appendix A — Incremental-pull feasibility for the remaining connectors

**Added**: 2026-06-02 · **Author**: Chris Hoefgen · **Status**: Analysis (not yet planned)

This appendix surveys the connectors **outside** part-3 scope — every connector that still inherits `supportsIncrementalPull() = false` after WordPress/Linear/HubSpot/Shopify/Intercom land — and judges whether each remote API can support the **server-side-predicate** archetype (answer "what changed since X?" during enumeration). The bar is the same one the rest of this plan uses: a server-side modified-since **filter**, sort-by-modified, cursor/delta token, or change feed on the **list endpoint(s) the connector actually calls**. Client-side filtering after a full enumeration is explicitly **not** an upgrade — it pages the whole table and saves no API calls (the same standard that kept Intercom articles/collections and the Webflow client-side design out of scope).

HubSpot and Shopify are omitted here — they are already part-3 scope. Webflow is included because, although deferred earlier, the analysis below shows its API does in fact support a server-side predicate (it was deferred for the *client-side* archetype that turned out to be unnecessary).

## A.1 Summary

| Connector       | Verdict     | Mechanism (endpoint + param → field)                                          | Sub-pattern                          | Complexity | Notes |
| --------------- | ----------- | ----------------------------------------------------------------------------- | ------------------------------------ | ---------- | ----- |
| **QuickBooks**  | ✅ Yes       | CDC `GET /cdc?entities=…&changedSince=<iso>` → `MetaData.LastUpdatedTime`      | fixed system field (all 23 entities) | Low–Med    | Different endpoint than the SQL-`query` path used today; 30-day CDC window, `FULL_PULL` is the long-dormant safety net |
| **Pipedrive**   | ✅ Yes       | `GET /v2/{deals,persons,organizations}?updated_since=<rfc3339>` → `update_time` | fixed system field (all 3 entities)  | Low        | **Verified**: installed `pipedrive@^31.2.1` v2 SDK exposes `updated_since` + `sort_by:'update_time'` on all three list calls |
| **Moco**        | ✅ Yes       | `GET /{companies,contacts/people,projects}?updated_after=<iso8601>` → `updated_at` | fixed system field (all 3 entities)  | Low        | `updated_after` documented for "almost all resources"; pair with delete-webhooks for deletions (out of scope, `FULL_PULL` reconciles) |
| **Brevo**       | 🟡 Partial  | `GET /contacts?modifiedSince=<iso>` → `modifiedAt` (contacts + templates only) | capability-gated by table            | Low        | Mailing lists have no modified timestamp → stay full-scan (Intercom-style per-table gate) |
| **Webflow**     | 🟡 Yes\*    | `listItems/assets/pages({ lastUpdated })` → `lastUpdated`                      | annotation-gated (Airtable-like)     | Medium     | API **does** support `lastUpdated`; was deferred for the client-side archetype that proves unnecessary. Per-collection gate on whether the schema exposes a last-modified field |
| **Attio**       | ❌ No\*      | query endpoints filter, but records expose only `created_at`, no `updated_at`  | —                                    | —          | Server-side filtering exists, but with no last-modified field it catches **creates, not edits** → not true incremental |
| **Affinity**    | ❌ No        | v2 `/persons /companies /opportunities /lists/{id}/list-entries` — no time filter | —                                 | —          | v1 had `changed_after` on lists; connector is v2-only and v2 has no equivalent yet |
| **Audienceful** | ❌ No        | `/people/` cursor list — no documented modified-since param                    | —                                    | —          | No `updated_at` field; API docs sparse — revisit if vendor confirms a filter |
| **Memberstack** | ❌ No        | `/members` cursor list — `limit`/`after`/`order` only                          | —                                    | —          | No last-modified field; webhooks are the only change signal (different archetype) |
| **Stripe**      | ❌ No        | list endpoints filter on `created[gte]` (creation), **not** last-modified      | —                                    | —          | Only change feed is the **Events API** — an opaque-cursor/change-feed archetype, not a list predicate; not implemented in the framework |
| **YouTube**     | ❌ No        | `playlistItems.list` (no time filter); `search.list` has `publishedAfter` only | —                                    | —          | `publishedAfter` = creation, not modification; no change feed |
| **Wix Blog**    | ❌ Blocked   | `pullRecordFiles` is currently a **stub** (returns empty)                       | —                                    | —          | Base pull not implemented; `queryDraftPosts` may support `editedDate` filters — re-evaluate after the base pull lands |
| **Generic API** | ⚠️ Deferred | user-declared `modifiedAtField` + configurable query-param name                | annotation-gated (user-configured)   | High       | Technically feasible but a foot-gun: misconfigured field/param → silent data loss. Recommend v2 with probe-validation, not v1 |

**Recommended next batch (all Low complexity, all the proven fixed-system-field pattern):** **Pipedrive, Moco, Brevo, QuickBooks**. Each is a near-mechanical repeat of the Linear/WordPress work in this plan — add a `*-incremental.ts` helper (predicate builder + 60s clock-skew constant), thread the param through the list call, override `supportsIncrementalPull`, flip `incrementalPull: true`. Webflow is a clear Medium-effort follow-up (Airtable-shaped annotation-gating). Everything else is genuinely blocked by the upstream API.

## A.2 Upgradeable — fixed system field (Low complexity)

### Pipedrive — `updated_since` on the v2 list endpoints

- **Entities**: deals, persons, organizations. **Current list**: `PipedriveApiClient.listEntities` cursor-paginates `dealsApi.getDeals` / `personsApi.getPersons` / `organizationsApi.getOrganizations` with only `{ limit, cursor }` ([pipedrive-api-client.ts:189-218](../../server/src/remote-service/connectors/library/pipedrive/pipedrive-api-client.ts#L189)). `pullRecordFiles` ignores `_options` ([pipedrive-connector.ts:110-127](../../server/src/remote-service/connectors/library/pipedrive/pipedrive-connector.ts#L110)).
- **Capability (verified)**: the already-installed `pipedrive@^31.2.1` v2 SDK exposes `updated_since` (RFC3339; "only deals with an `update_time` later than or equal to this time") plus `sort_by: 'update_time'` on **all three** `getDeals` / `getPersons` / `getOrganizations` requests (`node_modules/pipedrive/dist/versions/v2/api/{deals,persons,organizations}-api.d.ts`). No SDK bump, no raw-HTTP fallback.
- **Plan**: fixed system field, `supportsIncrementalPull() => true` for all three. Thread an optional `updatedSince` into `listEntities`; `pipedrive-incremental.ts` exports `PIPEDRIVE_INCREMENTAL_CLOCK_SKEW_MS = 60_000` + a `(since) => (since - skew).toISOString()` builder. `>=`/inclusive, watermark client-side, 60s margin; idempotent commits absorb the boundary re-pull.

### Moco — `updated_after` on every resource

- **Entities**: companies, contacts, projects, all via `MocoApiClient.listEntities` page-paginated ([moco-api-client.ts:301-327](../../server/src/remote-service/connectors/library/moco/moco-api-client.ts#L301)); `pullRecordFiles` at [moco-connector.ts:131-146](../../server/src/remote-service/connectors/library/moco/moco-connector.ts#L131). All three carry `updated_at` ([moco-types.ts:94,120,159](../../server/src/remote-service/connectors/library/moco/moco-types.ts)).
- **Capability**: Moco documents `updated_after` (ISO-8601 UTC) for "almost all resources" ([Moco API docs](https://everii-group.github.io/mocoapp-api-docs/)).
- **Plan**: fixed system field, unconditional. Add optional `updatedAfter` to `listEntities` (purely additive to the page pagination), `moco-incremental.ts` with the 60s constant + ISO builder. Deletions aren't covered by the filter — Moco recommends delete-webhooks; out of scope, `FULL_PULL` reconciles (consistent with the base plan's deletion philosophy).

### QuickBooks — Change Data Capture endpoint

- **Entities**: 23 entity types, all enumerated today via the SQL-like `POST /query` endpoint with `STARTPOSITION`/`MAXRESULTS` paging ([quickbooks-api-client.ts:91-107](../../server/src/remote-service/connectors/library/quickbooks/quickbooks-api-client.ts#L91)); `pullRecordFiles` at [quickbooks-connector.ts:127-151](../../server/src/remote-service/connectors/library/quickbooks/quickbooks-connector.ts#L127). Every QBO entity carries `MetaData.LastUpdatedTime`.
- **Capability**: Intuit's purpose-built incremental endpoint — `GET /v3/company/{realmId}/cdc?entities=<types>&changedSince=<iso8601>` ([QuickBooks CDC docs](https://developer.intuit.com/app/developer/qbo/docs/learn/explore-the-quickbooks-online-api/change-data-capture)).
- **Plan**: fixed system field, unconditional. Slightly more than a query param because CDC is a **different endpoint** with a different response envelope (one `CDCResponse` containing per-entity arrays) — add a `cdcSince(entities, since)` client method rather than threading a param into `query`. Two documented edges: CDC only spans the **last 30 days** (a folder dormant >30 days must fall back to full — the job already bootstraps full and `FULL_PULL` is the periodic safety net), and a single CDC response is capped (page/segment as the API allows). 60s clock-skew.

## A.3 Upgradeable — capability-gated / annotation-gated (Low–Medium)

### Brevo — `modifiedSince`, contacts + templates only

- **Entities**: contacts, email templates, mailing lists, each branched in `pullRecordFiles` ([brevo-connector.ts:111-168](../../server/src/remote-service/connectors/library/brevo/brevo-connector.ts#L111)); offset lists in [brevo-api-client.ts](../../server/src/remote-service/connectors/library/brevo/brevo-api-client.ts). Contacts and templates carry `modifiedAt` ([brevo-types.ts:15,75](../../server/src/remote-service/connectors/library/brevo/brevo-types.ts)); mailing lists do **not** ([brevo-types.ts:109-118](../../server/src/remote-service/connectors/library/brevo/brevo-types.ts#L109)).
- **Capability**: `GET /contacts?modifiedSince=<url-encoded UTC iso>` ([Brevo API ref](https://developers.brevo.com/reference/get-contacts)); the templates list takes the same filter.
- **Plan**: capability-gated by table (Intercom-shaped). `supportsIncrementalPull(_o, tableSpec)` returns true for `contacts` and `templates`, false for mailing lists (→ demote to full). Fixed field (`modifiedAt`), no resolver/advanced-setting. `brevo-incremental.ts` with the param builder + 60s constant.

### Webflow — `lastUpdated` filter (reverses the earlier deferral)

- **Entities**: collection items, assets, pages. **Current list**: `collections.items.listItems(collectionId, { offset, limit })` with no filter ([webflow-connector.ts:249-305](../../server/src/remote-service/connectors/library/webflow/webflow-connector.ts#L249)). Pages/assets already normalize a `lastUpdated` value ([webflow-connector.ts:60,336](../../server/src/remote-service/connectors/library/webflow/webflow-connector.ts#L60)).
- **Capability**: Webflow CMS API v2 supports server-side `lastUpdated` filtering on the list-items endpoint ([List Collection Items](https://developers.webflow.com/data/reference/cms/collection-items), [filtering/sorting changelog](https://developers.webflow.com/data/changelog/06182024)). **This is why the earlier deferral can be reversed** — the prior plan slotted Webflow as the *client-side-filter* archetype because it assumed no `modified_since` param; the param exists, so Webflow is a normal server-side-predicate connector, no new archetype needed.
- **Plan**: annotation-gated (Airtable-shaped) — auto-detect/annotate the last-modified field per collection, allow a `modifiedAtField` override, and per-collection-gate `supportsIncrementalPull` on whether the field resolves (collections whose schema exposes no last-modified field demote to full). Medium effort: schema annotation + resolver + filter builder, plus verifying the installed `webflow-api` SDK surfaces the `lastUpdated` filter param on `listItems`/`assets.list`/`pages.list` (confirm during implementation — same caveat the plan flags for the Linear `FILTER_TYPE_MAP`).

## A.4 Not upgradeable (with the current API)

- **Attio** — the query endpoints (`POST /v2/objects/{slug}/records/query`, `…/lists/{slug}/entries/query`) do support `$gt`/`$gte` date filters, **but the records the connector models expose only `created_at`, no `updated_at`** ([attio-types.ts:78,92,118,157,175](../../server/src/remote-service/connectors/library/attio/attio-types.ts)). Filtering on `created_at` would catch new records but miss edits — not a true incremental pull. Revisit if Attio exposes a record-level last-modified timestamp the connector can read.
- **Affinity** — built entirely on v2 endpoints (`/persons`, `/companies`, `/opportunities`, `/notes`, `/lists/{id}/list-entries`) ([affinity-connector.ts:217-293](../../server/src/remote-service/connectors/library/affinity/affinity-connector.ts#L217)); none accept a modified-since param, and persons/companies/opportunities expose no last-modified field. (The v1 API had `changed_after` on lists; the connector is v2-only.)
- **Audienceful** — single People table via cursor list ([audienceful-api-client.ts:93-110](../../server/src/remote-service/connectors/library/audienceful/audienceful-api-client.ts#L93)); no documented modified-since param and no `updated_at` field (only `created_at`/`last_activity`). Sparse API docs — flagged as *unconfirmed*; revisit if the vendor documents a filter.
- **Memberstack** — Members cursor list accepts only `limit`/`after`/`order` ([memberstack-api-client.ts:71-94](../../server/src/remote-service/connectors/library/memberstack/memberstack-api-client.ts#L71)); members carry `createdAt`/`lastLogin` but no last-modified field. Webhooks are the only change signal — a different archetype, out of scope.
- **Stripe** — list endpoints filter on `created[gte]` (creation time), **not** last-modified ([stripe-api-client.ts:98-130](../../server/src/remote-service/connectors/library/stripe/stripe-api-client.ts#L98)). The only update signal is the **Events API** (`*.updated` events), which is an opaque-cursor/change-feed pattern over a separate endpoint — not a list predicate, and not implemented in the framework. A genuine future option, but a new archetype.
- **YouTube** — the connector enumerates the uploads playlist via `playlistItems.list` (no temporal filter) then hydrates by id ([youtube-api-client.ts:28-96](../../server/src/remote-service/connectors/library/youtube/youtube-api-client.ts#L28)). `search.list` offers `publishedAfter`, but that is publish (creation) time, not modification, and there is no change feed for metadata edits.

## A.5 Special cases

- **Wix Blog — blocked on a missing base pull.** `pullRecordFiles` is currently a stub that immediately calls back with empty files ([wix-blog-connector.ts:146-156](../../server/src/remote-service/connectors/library/wix/wix-blog/wix-blog-connector.ts#L146)). Incremental is premature: the base full pull must be implemented first. When it is, `listDraftPosts` (paging only) likely won't carry filters, but the separate `queryDraftPosts` + Wix Query Language supports `$gt`/`$gte` on `editedDate` — so this could become an annotation-gated `editedDate` connector. Re-evaluate after the base pull lands.
- **Generic API — feasible but deliberately deferred.** No fixed schema or upstream API — each connection points at an arbitrary REST/GraphQL endpoint ([generic-api-connector.ts:225-290](../../server/src/remote-service/connectors/library/generic-api/generic-api-connector.ts#L225)). Incremental *could* be supported by adding two per-endpoint advanced settings — a user-declared `modifiedAtField` (which response field holds the timestamp) and a `modifiedSinceParam` (which query param the target API expects) — making it an annotation-gated, user-configured variant. The risk is a foot-gun: a wrong field/param name silently filters out changed records (data loss), with no way to validate against a known API. **Recommendation: not v1.** If pursued, gate it behind probe-time validation (only advertise incremental when a sample response actually contains the declared field and the param demonstrably narrows results) and clear misconfiguration errors.

## A.6 Verification notes

- **Pipedrive `updated_since`**: read directly from the installed SDK type definitions (`node_modules/pipedrive/dist/versions/v2/api/{deals,persons,organizations}-api.d.ts`) — all three list requests expose `updated_since`, `updated_until`, and `sort_by: 'id' | 'update_time' | 'add_time'`. This is the firmest of the "Yes" verdicts (no doc-only inference).
- **Attio / Affinity / Audienceful / Memberstack**: verdicts rest on the connector's own TypeScript models exposing no last-modified field, cross-checked against vendor docs. Audienceful in particular is *doc-sparse* — treat its "No" as "unconfirmed, no evidence of a filter" rather than a hard negative.
- **QuickBooks / Moco / Brevo / Webflow / Stripe / YouTube**: external-API capabilities confirmed against current vendor documentation (linked inline). For QuickBooks (CDC response envelope), Webflow (`webflow-api` SDK filter surface), and Brevo (templates-list filter parity), the exact request/response shape should be re-confirmed against the installed SDK at implementation time — the same "verify the input type during implementation" caveat the main plan applies to Linear's `FILTER_TYPE_MAP`.
