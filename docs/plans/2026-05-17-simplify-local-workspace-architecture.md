...
> 2. Sub-slice A is on master (commit `95329287`). B is in progress on `dev-10144-mr11` — read [`mr8-sub-slice-b-change-list.md`](mr8-sub-slice-b-change-list.md) for the per-step progress table and pickup notes (this is where the work resumes; the table below is preserved for the original spec).
> 3. Step 5 is the next slice to land — see the change-list's "Per-field upsert algorithm" section for the recommended approach to `accept_field` / `reject_field` / `run_discard_field`. Use `seed_accepted_patches_from_fixture` (already landed in step 4) when updating existing field-level tests.
> 4. Original step-order for steps not yet started: shared helpers first (`compute_accepted_state`, `apply_patch_entry_to_blob`) [DONE], then `run_accept` [DONE], `run_reject` [DONE], `run_discard` [DONE], then `_field` variants, then `_all` variants, then `_record` variants, then `upload`, then listing commands. Note the field-level rules from decision 34 — accept/reject act only on unapproved fields, discard acts on any state.
...
| Phase 4 + 5 — Retire `dirty` branch (merged)      | **In progress.** mr7 shipped re-anchor + apply helpers. mr8 sub-slice A (accepted-patches.json IO + compute_entry) shipped on master. mr11 sub-slice B in progress: steps 1–4 (foundation helpers + single-path commands) committed (WIP). C–G ahead — see [change-list](mr8-sub-slice-b-change-list.md) for per-step progress and pickup notes. |
