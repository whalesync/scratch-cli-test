...
> | F | Init rewrite: one non-sparse worktree per connection on `refs/heads/main`; drop sparse `dirty` and `master` worktrees. Best-effort migration of in-flight workspaces by diffing `dirty vs main`. | **Not started.** ~250 LOC. |
...
> 5. The original Phase 4 / Phase 5 sections below stay as the _design spec_; the slicing in this status block is the _execution plan_.
