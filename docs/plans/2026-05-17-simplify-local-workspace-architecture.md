...
> - **`mr8` sub-slice A (in flight, on branch — uncommitted)** — `accepted-patches.json` IO + accept-time diff helper, all gated under `#[allow(dead_code)]`:
...
> | ----- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ------------------ |
> | A     | `AcceptedPatchesFile` IO + atomic write + `compute_entry`. Pure helpers, no behavior change.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | ~250     | **landed locally** |
> | B     | Rewrite the 9 accept/reject/discard CLI commands in `files.rs` (lines ~911–1700, ~2153, ~3088–3392) to mutate `accepted-patches.json` instead of advancing `refs/heads/dirty` + `update_dirty_worktree_index`. New shared helper `compute_accepted_state(main_map, accepted_file) → FileMap` replaces today's `base_map = dirty_tree`. Touch points: `run_accept`, `run_accept_all`, `run_accept_field`, `run_reject`, `run_reject_all`, `run_reject_field`, `run_discard`, `run_discard_all`, `run_restore_deleted_record`, `run_discard_created_record`. Also rewrite `compute_unreviewed_entries`, `unreviewed_entries`, `accept_all_full_scan`, `reject_all_full_scan`, `discard_all_full_scan`, `discard_paths_single_repo`, and the `..._scoped_via_index` variants. **Goes together** — can't half-ship because mixing dirty-writes and JSON-writes would create inconsistent state. | ~600–800 | **NEXT**           |
...
> 2. Confirm `mr8` sub-slice A is on the branch (look for `scratch-git-2/src/cli/config/accepted_patches.rs` + the `compute_entry` function in `re_anchor.rs`). If not committed, commit it first as a self-contained foundation MR.
...
> 4. Then rewrite the 10 command entry points in dependency order: shared helpers first, then `run_accept`, then `run_reject`, then `run_discard`, then `_all` variants, then `_field` variants, then `_record` variants, then listing commands.
...
| Phase 4 + 5 — Retire `dirty` branch (merged)      | **In progress.** mr7 shipped re-anchor + apply helpers. mr8 sub-slice A (accepted-patches.json IO + compute_entry) landed locally. B–G ahead — see [Phase 4+5 status block](#phase-4--5--retire-dirty-branch-switch-to-accepted-patchesjson-merged-2026-05-19). |
