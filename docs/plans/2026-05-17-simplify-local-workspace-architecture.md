...
### Dogfood follow-ups (mr31, 2026-05-21)

Second dogfood pass against `wkb_3qH9SlxsNq` (Monorepo, 5 connectors, 135k files) focused on the publishing workflow. Four findings — three bugs fixed in-MR (D7 perf, D9 UX-block, D10 progress feedback), one design question open (D8 baseHead).

| #   | Item                                                                                                                                                                                                          | Why                                                                                                                                                                                                                                                                                                                                                                                                                          | Effort (CC) | Status                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D7  | `files upload` + `files publish` pre-flight: do per-connection work proportional to connection count, not to "did this connection actually have anything to upload?"                                          | Measured 99.5s wall time on `files upload` against the Monorepo when only Airtable had patches (2 files). Per-connection `upload_single_repo_via_patches` was running `fetch_origin` + `read_main_tree` + `read_materialized_repo` + `compute_unreviewed_entries` *before* checking whether `accepted-patches.json` was empty. Same shape on `run_publish` — no pre-flight at all, so unreviewed edits would silently miss the upload. | 2h          | **Shipped on `mr31`** — (1) new `detect_unreviewed_fast(ctx, short_circuit)` in `cli/commands/files.rs` uses `gix::Repository::status` (index-backed, ~210ms warm per connection) + a small post-filter against `accepted-patches.json` for paths that are both dirty AND patched. (2) wired as `run_publish` pre-flight → returns the same `blocked_unreviewed` structured payload as pull. (3) `upload_single_repo_via_patches` empty-check moved to the top (skips fetch + tree walks); the `fetch_origin` call dropped entirely (it made `baseHead` equal to `refs/remotes/origin/main` by construction, so the server's staleness check could never fire); slow unreviewed warning replaced with the fast helper. Measured: upload 99.5s → 2.9s (~34×); publish pre-flight 1.7s vs old equivalent path's 93s (~55×). |
| D8  | `baseHead` divergence on `/upload-patch/commit`: with the fetch dropped (D7), `baseHead = local refs/heads/main` is now meaningful. What should happen when it ≠ remote `main`?                               | Two coherent designs: (A) keep current "apply anyway + `stalenessWarning` soft signal" (decision log #4); (B) refuse-and-require-`files download` first — symmetric with pull's `blocked_unreviewed`, free implementation via existing `re_anchor_patches`, surfaces conflicts visibly in `.scratch/conflicts.log` instead of burying them. The publish gate (D7) means the user is necessarily "fresh" when they hit Publish, lowering (B)'s friction cost significantly. | 1-2h        | Open. Recommended path: (B) plus a `refuseIfStale: true` flag on `/upload-patch/commit` (default false for back-compat). Decision deferred — needs decision log update + server-side change. See also follow-up E5 (lighting up `stalenessWarning` server-side) — now half-done by D7's `baseHead` fix.                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| D9  | Desktop's PublishChangesModal showed "N records contain unreviewed changes that will not be uploaded. Continue to upload the reviewed local changes, or cancel and review those edits first." with a single "Continue" button — letting users proceed past unreviewed edits silently | Symmetric with pull's three-button refuse-modal, the publish modal needed the same block-with-actionable-choice UX. Once shipped, the user can never "Continue past" unreviewed edits — they have to actively decide what to do with them first. The CLI publish pre-flight (`run_publish` → `blocked_unreviewed`) is the terminal-user safety net; the desktop hits its own modal gate because the renderer publishes via direct HTTP (`publishApi.startPlanJob` / `startRunJob`), not via `scratchmd files publish`. | 30min       | **Shipped on `mr31`** — `PublishChangesModal.tsx` `'approval'` mode swaps the single "Continue" button for a three-button row ("Discard and publish" / "Accept and publish" / Cancel) whenever `unreviewedEntries.length > 0`. New `handleAcceptAllAndUpload` / `handleDiscardAllAndUpload` callbacks shell out to the existing `acceptAllChanges` / `discardAllChanges` IPC handlers, then `startUpload()`. The validation-problems-only path keeps the "Ignore and Continue" button unchanged. Copy revised: "Publishing is blocked until you decide what to do with these edits." **Follow-on**: `triggerPublish` (the "Publish now" click handler in the `'uploaded'` mode) now re-runs `listUnreviewedChanges` before firing `publishApi.startPlanJob`. If anything appeared between upload and click (file watcher, external edit, another window), the modal flips back to `'approval'` so the user resolves the new edits before retrying — closes the only remaining gating gap given the renderer's direct-HTTP publish path. |
| D10 | PublishChangesModal showed a bare spinner during multi-second pre-flight phases (~1.7s `listUnreviewedChanges` + validation-stat lookup at open, ~3s upload, ~1.7s re-check before publish). No visibility into "what is the modal actually doing right now" — silent spinners feel slower than they are, and on a 135k-file workspace the user was left wondering whether anything was happening | Perception of activity is half the win. Showing concrete steps with check-mark transitions ("Checking for unreviewed local edits → Found 1 record → Loading validation status → No validation issues → Uploading accepted edits to the server → Uploaded 2 records to 1 connection") gives the user a frame-by-frame trace of work, matches the `'publishing'` mode's per-connection progress rows in style, and pays only ~100ms of sequential ordering (vs. the prior `Promise.all`) for the legibility win | 1h          | **Shipped on `mr31`** — new `ProgressStep` type + `ProgressStepList` component renders a checklist of steps with `Loader` (active), `CheckCircle2` (done), `Circle` (pending), `XCircle` (error) icons. `loadInitialState` now runs the two checks sequentially with step transitions; `handleAcceptAllAndUpload` / `handleDiscardAllAndUpload` add their own active step; `startUpload` appends an "Uploading accepted edits to the server" step with a finalized "Uploaded N records to M connections" label; `triggerPublish` shows a brief "Re-checking for unreviewed local edits" step before flipping to `'publishing'`. Replaces both the bare spinner during `initializing` and the generic "Uploading changes to the server / This usually takes a few seconds." block in `'uploading'` mode. |
