...
**Status**: Phases 1–3 shipped on `mr1`/`mr2`/`mr3` (Phase 1), `mr4` (Phase 2), `mr6` (Phase 3). Phase 4+5 merged (see [decision log](#phase-45-merger-2026-05-19)). Phase 4+5 sub-slice A (`accepted-patches.json` IO + `compute_entry`/`re_anchor`/`apply` helpers) shipped on `mr7`/`mr8`. Sub-slice B (the full cutover of accept/reject/discard/upload/listing commands to `accepted-patches.json`) shipped across `mr11` (steps 1–4), `mr12` (step 5), `mr13` (steps 6–12) and `mr14` (step 13 dogfood + bugfix + REVIEW_MODEL.md doc). Sub-slice B steps 5–13 done — see [change-list](mr8-sub-slice-b-change-list.md) for the per-step record. The pull rewrite (slice D), `folder_index` column recompute (slice E), [shared Rust library + desktop migration](#slice-h--shared-rust-library--desktop-migration) (slice H), workspace-init collapse to one non-sparse worktree on `main` (slice F, blocked on H), Phase 6 (parallelize connections), and Phase 7 (delete `run-from-git`) are not started. Two follow-ups surfaced during dogfood: [DEV-10175](https://linear.app/whalesync/issue/DEV-10175/scratchmd-files-publish-clears-accepted-patchesjson-even-when) (publish clears patch file on orchestrator-level success regardless of connector failure) and slice H (the desktop's five mutating cell handlers still write directly to `refs/heads/dirty` from TypeScript, bypassing sub-slice B).
...
> **Status: SUB-SLICES A + B + C + G SHIPPED (2026-05-20).** Sub-slices D (pull stash/replay), E (`folder_index` column compute), and F (init collapse to one `main` worktree) are not started. Single-user accept/reject/discard now flow through `accepted-patches.json`; `files upload` ships the file verbatim; `files publish` clears it on success; listing commands derive from it. The user-facing worktree is still on `refs/heads/dirty` (sparse) until F retires that indirection. See [Phase 4+5 merger](#phase-45-merger-2026-05-19) in the decision log for the architectural context.
>
> **What shipped:**
>
...
>   - `files publish` clears the file post-`refs/heads/main` advance.
...
>   - **Follow-up bug filed:** [DEV-10175](https://linear.app/whalesync/issue/DEV-10175/scratchmd-files-publish-clears-accepted-patchesjson-even-when) — `files publish` clears `accepted-patches.json` on orchestrator-level success even when the underlying connector batch failed.
>
> **Slice status:**
>
...
> | D     | Rewrite `download_single_repo` as stash/replay (`working-patches.json` + `re_anchor_patches` against new HEAD). `--clear-stash` recovery flag. `.scratch/conflicts.log` + `desktop.pull.conflict` PostHog event.                                                                                                                                                                                                                                                                                      | **Not started.** ~300–400 LOC.                                                      |
> | E     | `folder_index` columns `approvedChanges` / `unapprovedChanges` switch compute: `approvedChanges = path appears in accepted-patches.json`; `unapprovedChanges = working differs from apply(snapshot, accepted_patch_for_path)`. Same columns + SQL — only population logic changes.                                                                                                                                                                                                                    | **Not started.** ~80 LOC.                                                           |
> | F     | Init rewrite: one non-sparse worktree per connection on `refs/heads/main`; drop sparse `dirty` and `master` worktrees. Best-effort migration of in-flight workspaces by diffing `dirty vs main`.                                                                                                                                                                                                                                                                                                      | **Not started — blocked on H.** ~250 LOC.                                           |
> | G     | Listing commands derive from `(main, accepted-patches.json, working)`.                                                                                                                                                                                                                                                                                                                                                                                                                                | **Absorbed into sub-slice B step 9.**                                               |
> | H     | **Shared Rust library + desktop migration.** Hoist the I/O-free accept/reject/discard core out of `cli/commands/files.rs` into `shared/`; expose via a new `napi-rs` cdylib crate; rewrite the 5 direct-dirty-ops handlers in `scratch-desktop/src/main/local-files.ts` to call the Rust functions instead of patching the dirty worktree + advancing the dirty ref from TypeScript. See [Slice H — Shared Rust library + desktop migration](#slice-h--shared-rust-library--desktop-migration) below. | **Not started — blocks F.** ~600–800 LOC (Rust ~300 + TS ~200 + CI/packaging ~150). |
>
> **Pickup notes for the next slice:**
>
> 1. Pre-F, the user-facing worktree is still `ctx.dirty_dir` (sparse on `refs/heads/dirty`). Accept/reject/discard write to `accepted-patches.json` but the worktree path is unchanged. F is what retires the worktree-path indirection.
> 2. `re_anchor::re_anchor_patches` is wired up but only exercised by unit tests today — slice D is the production caller.
...
> 4. The follow-up bug [DEV-10175](https://linear.app/whalesync/issue/DEV-10175/scratchmd-files-publish-clears-accepted-patchesjson-even-when) is best addressed alongside slice D — option 1 in its fix list ("re-anchor instead of clear") uses the same `re_anchor_patches` machinery slice D needs.
> 5. **Slice F is blocked on slice H** — F retires `refs/heads/dirty` from init; but today five Electron handlers (`acceptCellChange`, `acceptCellInputText`, `undoApprovedCellChange`, `restoreDeletedRecord`, `discardCreatedRecord` in `scratch-desktop/src/main/local-files.ts`) write directly to the dirty ref. F would break the desktop until H lands.
> 6. The original Phase 4 / Phase 5 sections below stay as the _design spec_; the slicing in this status block is the _execution plan_.
...
> **Surfaced 2026-05-20 during sub-slice B dogfood.** The desktop's `scratch-desktop/src/main/local-files.ts` has five mutating handlers that bypass `scratchmd` entirely — they patch the dirty worktree, the working file, and the dirty git ref directly from Node, duplicating the CLI's accept/reject/discard logic in TypeScript. This was a deliberate choice in [Ivan's April 7 "new diffing UI" PR (`ff5b1529`)](https://gitlab.com/whalesync/spinner/-/commit/ff5b15296ed6a165829ea696c587a1a4cc6f4fb5) to avoid the ~50ms per-call cost of spawning `scratchmd` from Electron when a user types in a grid cell. The cost paid for that latency win: two implementations of the same domain logic in two languages — and after sub-slice B, the CLI now writes to `accepted-patches.json` while the desktop still writes to `refs/heads/dirty`. The two surfaces are now semantically incompatible.

**Goal:** Replace per-language duplication with a shared Rust library callable from both the CLI binary and the Electron main process via [napi-rs](https://napi.rs/) — same model that powers `next/swc`, `oxc`, Parcel's Rust plugins, and Sentry's CLI tooling. The desktop calls into Rust like any other JS function — no spawn, no IPC, sub-millisecond round-trip.

**Surface — the five handlers to migrate:**

| Handler in `scratch-desktop/src/main/local-files.ts` | Today                                                  | After slice H                                           |
| ---------------------------------------------------- | ------------------------------------------------------ | ------------------------------------------------------- |
| `acceptCellChange` (1709)                            | Patch working + reviewed-dirty file, advance dirty ref | Call shared Rust `accept_field`                         |
| `acceptCellInputText` (1727)                         | Same with schema-driven type coercion                  | Same; schema coercion stays in TS, value passes to Rust |
| `undoApprovedCellChange` (1773)                      | Restore field from master into working + dirty + ref   | Call shared Rust `discard_field`                        |
| `restoreDeletedRecord` (1810)                        | Copy master → working + dirty + ref                    | Call shared Rust `restore_deleted_record`               |
| `discardCreatedRecord` (1831)                        | Remove working + dirty + ref                           | Call shared Rust `discard_created_record`               |

All five become thin TypeScript shims around napi calls.
...
├── shared/                       # cross-cutting logic (already exists)
│   ├── review_ops.rs (new)       # hoist accept/reject/discard cores out of cli/commands/files.rs
│   ├── layout.rs
│   ├── folder_index.rs
│   └── ...
├── cli/                          # binary, depends on shared
├── service/                      # binary, depends on shared
└── napi/                         # NEW: cdylib crate, depends on shared
    ├── Cargo.toml                # crate-type = ["cdylib"]
    └── src/lib.rs                # napi-rs #[napi] bindings exposing review_ops fns to Node
```

`cli/commands/files.rs::run_accept` / `run_accept_field` / etc. become thin command wrappers around `shared::review_ops::*`. Most of the I/O-free helper work was already done as part of sub-slice B (`compute_accepted_state`, `apply_patch_entry_to_blob`, `accept_field_in_folder`, etc.) — slice H is mostly a directory move + a new binding crate, not a re-derivation.

**Cross-cutting concerns:**

- **Per-platform prebuilds.** napi-rs emits a `.node` file per `(os, arch)` — Mac arm64, Mac x64, Linux x64 minimum; Windows when we support it. napi-rs's GitHub Actions templates handle the build matrix + npm publishing. The desktop's `electron-builder` config bundles the matching prebuild for the target. The bundled `scratchmd` binary in `Resources/bin/` already faces this problem (it shipped a 5-day-stale version during dogfood — see [`mr14`](#what-shipped)), so adding a `.node` to the same pipeline isn't new ground.
- **Error handling.** napi-rs maps Rust `Result<T, E>` to JS exceptions cleanly. The binding layer wraps all `accept/reject/discard` calls in `Result`; no `unwrap()` on user input; panics map to thrown JS errors instead of crashing Electron's main process. Each binding gets a smoke test that verifies error paths surface as exceptions.
- **Workspace lock.** The CLI acquires `<workspace>/.scratch/lock` for any mutating op. The desktop's direct handlers today don't — they rely on the user not running CLI ops concurrently. The shared Rust library will acquire the lock on every entry point, so the desktop inherits the same discipline automatically. Resolves CLI-review follow-up [C1](#cli-review-follow-ups).
- **Memory + lifecycle.** The native addon shares Electron's main process address space. The binding layer doesn't hold global state — every call opens what it needs, returns, releases. No daemon threads.
- **TS types.** napi-rs generates `.d.ts` from the Rust signatures; no hand-written wire types. Removes a class of drift bug that already exists between the CLI's JSON output and the desktop's parsers.
- **Test strategy.** Rust core has its existing unit-test coverage from sub-slice B. The binding crate gets thin smoke tests that exercise the JS → Rust → JS round-trip. The desktop's Jest suite stays at the handler level — each rewritten handler gets a "calls the napi binding with the expected args, returns the expected shape" test.

**Sequencing:**

- **Before H:** sub-slice B is in. The CLI writes to `accepted-patches.json`; the desktop writes to `refs/heads/dirty`. The two surfaces produce inconsistent local state.
- **H:** Desktop migrates. Both surfaces now write to `accepted-patches.json` via the same Rust core.
- **After H, before F:** Workspace is fully on the new model. The dirty ref still exists on disk (used by init / download), but nothing writes to it from local actions.
- **F:** Init stops creating the dirty worktree at all. Safe now that no live surface depends on it.

**Why this is in the plan and not just a follow-up:** the duplicated TypeScript handlers are a load-bearing piece of the post-Phase-5 architecture — slice F can't ship cleanly until they're gone. We could pretend it's an unrelated cleanup, but that would leave Phase 5 partially-shipped indefinitely. Promoting H to a plan slice forces it into the same gating discipline as the rest.

**Done when:**

- The `napi/` crate builds, publishes per-platform `.node` files, and is bundled into the Electron app.
- All five handlers in `scratch-desktop/src/main/local-files.ts` are thin shims calling the napi bindings.
- Editing a cell in the desktop app produces an entry in `<workspace>/.scratch/connections/<conn>/accepted-patches.json` (dogfood-verifiable).
- The desktop no longer writes to `refs/heads/dirty` (grep `commitReviewedDirtyFile` returns nothing in `scratch-desktop/`).
- `cargo test`, `cargo build`, `yarn lint-strict` in `server/` + `scratch-desktop/` all green.
...
| Phase 4 + 5 — Retire `dirty` branch (merged)      | **Sub-slices A + B + C + G shipped** on `mr7`→`mr14` (2026-05-19 → 2026-05-20). Single-user accept/reject/discard now write to `accepted-patches.json`; `files upload`/`publish` consume it; listing commands derive from it; ~600 LOC of three-worktree logic deleted. **Slices D (pull stash/replay), E (`folder_index` column compute), H (shared Rust library + desktop migration), F (init collapse to one `main` worktree — blocked on H) not started.** Dogfood pass on `mr14` surfaced [DEV-10175](https://linear.app/whalesync/issue/DEV-10175/scratchmd-files-publish-clears-accepted-patchesjson-even-when) (publish over-eagerly clears the file) and the desktop-bypass problem that slice H addresses. See [Phase 4+5 status block](#phase-4--5--retire-dirty-branch-switch-to-accepted-patchesjson-merged-2026-05-19) for slice-by-slice detail and [`mr8-sub-slice-b-change-list.md`](mr8-sub-slice-b-change-list.md) for the per-step record. |
...
| ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | --------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [DEV-10175](https://linear.app/whalesync/issue/DEV-10175/scratchmd-files-publish-clears-accepted-patchesjson-even-when) | `scratchmd files publish` clears `accepted-patches.json` even when connector batch fails | Sub-slice B dogfood, `mr14` | Best fixed alongside slice D — option 1 in the ticket's fix list ("re-anchor instead of clear") reuses `re_anchor::re_anchor_patches`, the same machinery slice D needs. Medium priority. |
...
The slice is captured in the [Phase 4+5 status block](#phase-4--5--retire-dirty-branch-switch-to-accepted-patchesjson-merged-2026-05-19) as **Slice H**; the implementation spec lives in [Slice H — Shared Rust library + desktop migration](#slice-h--shared-rust-library--desktop-migration). Slice F (init retires the dirty branch) is now blocked on H — without H, the desktop would lose its read/write surface on the day F lands.
