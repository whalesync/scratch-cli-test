...
**Status**: Phase 1 shipped on `dev-10144-{mr1,mr2,mr3}`. Phase 2 shipped on `dev-10144-mr4`. Phase 3 shipped on `dev-10144-mr6`. Phase 4 in progress on `dev-10144-mr7` (re-anchor helper landed; pull rewrite next). Phases 5–7 not started.
...
> **Status: IN PROGRESS** on `dev-10144-mr7`. Sub-slice 1 — the pure re-anchor helper plus RFC 7396 `apply()` — has landed locally (not yet committed). Added: `scratch-git-2/src/cli/commands/re_anchor.rs` (new module: `PatchKind`, `AnchoredPatch`, `PatchConflict`, `ReAnchoredOne`, `ReAnchorOutput`, `re_anchor_one`, `re_anchor_patches`; 15 unit tests covering server-untouched paths, no-op detection, path-deleted kind conversion, same-field collision, disjoint changes, user-delete-on-modified-path, batched API, serde shapes); `scratch-git-2/src/cli/commands/merge_patch.rs::apply` (mirror of the server's `applyJsonMergePatch`, 7 tests including a diff-apply round trip). Both are gated under `#[allow(dead_code)]` until the pull rewrite (sub-slice 3) wires them in. Verified: `cargo build` clean (0 warnings), `cargo test` green (233 passes), `cargo fmt` applied. Remaining sub-slices for `mr7`: (2) a `gix::Repository::status`-based helper to enumerate user-modified worktree files; (3) rewrite the download flow to write `working-patches.json` → fetch → re-anchor + replay; (4) wire `--clear-stash` flag and the desktop "Show file in Finder/Explorer" affordance.
...
| Phase 4 — Pull stash/replay                       | **In progress** on `dev-10144-mr7`. Re-anchor helper + RFC 7396 `apply` landed locally; pull rewrite next.         |
