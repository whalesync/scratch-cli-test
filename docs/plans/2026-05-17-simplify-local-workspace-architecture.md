...
> | H     | **Shared Rust library + desktop migration.** Hoist the I/O-free accept/reject/discard core out of `cli/commands/files.rs` into `shared/review_ops.rs`; expose via a new `napi-rs` cdylib crate; rewrite the 3 direct-dirty-ops handlers in `scratch-desktop/src/main/local-files.ts` (cell-edit hot path: `acceptCellChange`, `acceptCellInputText`, `undoApprovedCellChange`) to call the Rust functions instead of patching the dirty worktree + advancing the dirty ref from TypeScript. Detailed spec: [`2026-05-20-slice-h-spec.md`](2026-05-20-slice-h-spec.md). Summary: [Slice H — Shared Rust library + desktop migration](#slice-h--shared-rust-library--desktop-migration).                                                                                                                                                                                                                                                                                                                                               | **H.1 + H.1.5 shipped** (H.1: `mr20`/`30010d41`, 2026-05-20; H.1.5: 2026-05-20 — `shared/git_local.rs`, `workspace_lock` moved to `shared/`, four public entry points + types, +5 unit tests). H.2/H.3/H.4 not started — blocks F. Total ~600–800 LOC. |
>
...
> 5. **Slice F is blocked on slice H** — F retires `refs/heads/dirty` from init; but today three live Electron handlers (`acceptCellChange`, `acceptCellInputText`, `undoApprovedCellChange` in `scratch-desktop/src/main/local-files.ts`) write directly to the dirty ref. F would break the desktop until H.3 lands. The Rust core they'll call is now ready (H.1.5 shipped the four `shared::review_ops::*` entry points + `LockMode` + `ReviewOpResult`); H.2 wires napi, H.3 swaps the JS handlers.
...
- **H.2/H.3:** Desktop's three cell-edit handlers migrate to napi. Both surfaces now write to `accepted-patches.json` via the same Rust core.
