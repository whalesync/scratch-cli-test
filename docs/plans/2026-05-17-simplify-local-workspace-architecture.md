...
> **Status: SUB-SLICES A + B + C + D + G SHIPPED (2026-05-20).** Sub-slices E (`folder_index` column compute) and F (init collapse to one `main` worktree, blocked on H) are not started; H itself is not started. Single-user accept/reject/discard flow through `accepted-patches.json`; `files upload` ships the file verbatim; `files publish` clears it on success; listing commands derive from it. **`scratchmd files download` now refuses with a structured `blocked_unreviewed` error when any working-tree edits are unreviewed; on the happy path it fetches origin, re-anchors `accepted-patches.json` against the new server `main`, replays the patches on top of the new blobs, and advances local `refs/heads/main`. Same-field collisions land in `.scratch/conflicts.log` (user wins).** The user-facing worktree is still on `refs/heads/dirty` (sparse) until F retires that indirection. See [Phase 4+5 merger](#phase-45-merger-2026-05-19) in the decision log for the architectural context.
>
> **What shipped:**
>
...
> 4. The follow-up bug [DEV-10175](https://linear.app/whalesync/issue/DEV-10175/scratchmd-files-publish-clears-accepted-patchesjson-even-when) is still open — D landed the `re_anchor_patches` infrastructure that option 1 of the fix list ("re-anchor instead of clear") needs, so the fix is now mechanical: call `re_anchor_patches` in `files publish` post-fetch instead of unconditionally clearing.
...
> 6. **`download_workbook` still bypasses the lock + pre-flight check** (called from `linked.rs` after server-side mutations). Unreviewed working-tree edits would be silently overwritten here. Low-risk in practice (users doing linked-table CRUD aren't mid-edit), but worth tightening in a follow-up — add the same `detect_unreviewed_for_pull` pass and either warn-and-skip or surface a structured error.
...
| ----------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [DEV-10175](https://linear.app/whalesync/issue/DEV-10175/scratchmd-files-publish-clears-accepted-patchesjson-even-when) | `scratchmd files publish` clears `accepted-patches.json` even when connector batch fails | Sub-slice B dogfood, `mr14` | Slice D wired `re_anchor::re_anchor_patches` into production, so option 1 of the ticket's fix list ("re-anchor instead of clear") is now mechanical: call it from `files publish` post-fetch instead of unconditionally clearing. Medium priority. |
