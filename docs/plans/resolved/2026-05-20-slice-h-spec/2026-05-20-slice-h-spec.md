...
**Date**: 2026-05-20
**Status**: **H.1 + H.1.5 + H.2 + H.3 + H.4 shipped on `dev-10144`** (H.1: `mr20`/`30010d41`; H.1.5–H.3: 2026-05-20; H.4: 2026-05-21 on `mr27`). Slice H complete. See [Sequencing](#sequencing-inside-the-slice) for what landed in each sub-slice.
**Parent plan**: [2026-05-17-simplify-local-workspace-architecture.md → Slice H](../2026-05-17-simplify-local-workspace-architecture/2026-05-17-simplify-local-workspace-architecture.md#slice-h--shared-rust-library--desktop-migration)
...
[Slice F](../2026-05-17-simplify-local-workspace-architecture/2026-05-17-simplify-local-workspace-architecture.md) (init collapse to one `main` worktree) cannot ship until the desktop stops touching `refs/heads/dirty`. H is the gating slice.

The chosen approach (from [decision log → Shared Rust library via napi-rs](../2026-05-17-simplify-local-workspace-architecture/2026-05-17-simplify-local-workspace-architecture.md#shared-rust-library-via-napi-rs-not-duplicated-typescript-2026-05-20)) is to hoist accept/reject/discard cores out of `cli/commands/files.rs` into a new `shared::review_ops` module, then expose those functions to Node via a [napi-rs](https://napi.rs/) cdylib crate. Both the CLI binary and the Electron main process consume the same Rust core. Native addon round-trip is sub-millisecond — no spawn, no IPC to a subprocess.
...
Side benefit: this closes [CLI-review follow-up C1](../2026-05-17-simplify-local-workspace-architecture/2026-05-17-simplify-local-workspace-architecture.md#cli-review-follow-ups) for the field-level mutations — `review_ops`'s public entry points wrap the lock, so the CLI wrappers inherit it automatically (they already call `acquire` today; the change is moving the acquire site into `review_ops` instead of the wrapper).
...
**Risk to watch:** the existing `Resources/bin/scratchmd` binary has shipped stale at least once ([`mr14` dogfood note](../2026-05-17-simplify-local-workspace-architecture/2026-05-17-simplify-local-workspace-architecture.md#what-shipped)) because the desktop's CI didn't pull a fresh build. Same failure mode applies to `.node` files. Mitigation: bake a build-time SHA check into the desktop's CI — fail the build if the bundled binary/`.node` SHAs don't match the latest cargo build artifact.
...
**H.4 — Multi-platform CI + afterPack wiring. ✅ Shipped 2026-05-21 (`dev-10144-mr27`).** The CI `.build_cli_for_desktop` job now produces a `.node` for every platform the desktop ships to (Mac arm64 + Linux x64). Both files flow through the same `cli-binaries/<rust-target>/` artifact tree the existing `scratchmd` binary uses, and the desktop's `afterPack.cjs` hook copies the platform-correct `.node` into the packaged `.app`'s `Resources/bin/`. This replaces the previous `electron-builder.yml` `extraResources` glob (which would have shipped stale dev-machine `.node` files on platforms where the build runner had no cargo).

- **`scratch-desktop/.gitlab-ci-release.yml::.build_cli_for_desktop`** — after each `cargo zigbuild --release --bin scratchmd --target <triple>`, also runs `cargo zigbuild --release -p scratchmd-native --target <triple>` for the two supported targets (`aarch64-apple-darwin`, `x86_64-unknown-linux-gnu`). The resulting `.dylib`/`.so` is renamed in-place to `cli-binaries/<triple>/scratchmd-native.<platform>-<arch>[-<abi>].node` so the file matches the loader's expected filename exactly. Windows napi deferred — same surface as before slice H (desktop cell-edits on Windows throw at runtime; tracked as a follow-up).
- **`scratch-desktop/scripts/afterPack.cjs`** — extended to copy the platform-correct `.node` from `cli-binaries/<triple>/` into `<resourcesDir>/bin/` alongside the CLI binary. On `win32-x64` the napi copy is skipped with a logged warning (CLI still gets copied). On other supported platforms, a missing `.node` in `cli-binaries/` raises a loud failure with a copy-paste cargo command. Both files get `chmod 755` and land in the bundle before electron-builder's codesign pass.
- **`scratch-desktop/electron-builder.yml`** — `extraResources` glob for `../scratch-git-2/napi/*.node` removed. afterPack is the canonical path; comment in the YAML records why. `electron-builder.unsigned-mac.yml` inherits via `extends:` so the change propagates.
- **Local mac scripts** — `scripts/build_mac_local.sh` and `scripts/build_mac_prod_local.sh` now build both `scratchmd` and `scratchmd-native` for `aarch64-apple-darwin` and drop both into `cli-binaries/aarch64-apple-darwin/` before invoking `yarn build:mac:unsigned` / `package.sh mac`. The native-Mac build uses plain `cargo build` (matching the existing CLI heuristic — zigbuild's `SDKROOT/-L` handling fights Xcode on a host Mac). `build_mac_prod_local.sh`'s `BUILD_SCRATCHMD=0` short-circuit now requires both files to exist before reusing the cached state. `build_mac_local_signed.sh` left untouched — it's a sign-and-package wrapper that assumes `cli-binaries/` is already populated by one of the other scripts.
- **No SHA check baked in.** The spec's original "stale `.node` SHA guard" was scoped to the `Resources/bin/` in-repo bundling approach, which we did not adopt. With afterPack pulling fresh CI-built artifacts every release, there is no in-repo bundled copy that could go stale. The loud-failure-on-missing in afterPack covers the "did CI build it?" case directly.
- **macOS notarization** — electron-builder runs `afterPack` _before_ the codesign + notarize pass, so the copied `.node` is included in the signed bundle and goes through notarization automatically. Verified by dry-running afterPack against a fake context (`/tmp/afterpack-dryrun/Scratch.app/Contents/Resources/bin/`) — both `scratchmd` and `scratchmd-native.darwin-arm64.node` land in the expected location. Full end-to-end notarization is verified on the user's next packaged-mac release.
- **End state:** Desktop `yarn lint` + `yarn build` + `yarn test` (140 tests) clean; napi smoke tests (`node --test napi/__tests__/*.mjs`) still 5/5 green; YAML parses (electron-builder.yml + unsigned-mac.yml + the CI release file with GitLab `!reference` tags); afterPack copies exercised for all 3 packaged platforms in dry-run + the missing-`.node` error path.

Slice H complete. The shared Rust core, the napi binding crate, the desktop loader + handler migration, and the packaging pipeline are all in production. Future Rust-core moves (validators, schema readers, folder-index queries) follow the same template.
...
- **Desktop CI shipping stale prebuilds.** Already seen on `Resources/bin/scratchmd` ([`mr14` dogfood](../2026-05-17-simplify-local-workspace-architecture/2026-05-17-simplify-local-workspace-architecture.md#what-shipped)). Same failure mode applies to `.node` files. Mitigation in [build pipeline](#build-and-distribution-pipeline): bake a SHA check into the desktop's CI.
...
- ✅ The packaged `.app`'s `Contents/Resources/bin/` (or `resources/bin/` on Linux) contains `scratchmd-native.<platform>-<arch>[-<abi>].node` files for the supported platforms (Mac arm64 + Linux x64). `scratch-desktop/scripts/afterPack.cjs` copies them from `scratch-git-2/cli-binaries/<rust-target>/` — populated by the CI `.build_cli_for_desktop` job. The previous `electron-builder.yml` `extraResources` glob is gone. **(H.4)**
...
- ⏳ Editing a cell in a packaged desktop build produces a new entry in `<workspace>/.scratch/connections/<conn>/accepted-patches.json` (dogfood-verified on at least one connector) and does NOT advance `refs/heads/dirty`. Deferred to user-run dogfood on the next packaged mac build (now that H.4 has wired the napi `.node` into the .app); standalone Node smoke tests already prove the JS → Rust → JS round trip end-to-end. **(H.3 + H.4)**
...
- ⏳ macOS notarization passes on the packaged build with the `.node` file signed. afterPack copies the `.node` into the bundle BEFORE electron-builder's codesign + notarize pass, so structurally it's covered; full e2e verification ships with the user's next packaged-mac release. **(H.4)**
...
- **Future Rust-core moves** — validators, schema readers, folder-index queries are all under the same "duplicated across CLI and desktop" pressure. With napi as the canonical pattern, they get migrated the same way. See [decision log → Shared Rust library via napi-rs](../2026-05-17-simplify-local-workspace-architecture/2026-05-17-simplify-local-workspace-architecture.md#shared-rust-library-via-napi-rs-not-duplicated-typescript-2026-05-20)'s "stronger principle."
- **CLI workspace lock coverage** — `shared::review_ops` acquires the lock on every entry, which closes CLI-review follow-up [C1](../2026-05-17-simplify-local-workspace-architecture/2026-05-17-simplify-local-workspace-architecture.md#cli-review-follow-ups) for the field-level commands. The remaining `*-all` commands still rely on implicit serialization; revisit when slice F retires the worktree-per-branch model entirely.
