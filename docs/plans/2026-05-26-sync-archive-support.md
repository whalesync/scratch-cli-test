...
- **Lane E (tests)** — T15, T16 (v1-compat regression specs in `server/test/integration/sync-mapping-v1-compat.spec.ts`), T20 (CI `sync_v1_compat` job). Now also covers an end-to-end Pass 3 integration spec exercising the archive worked example from DEV-10008, plus a DB-backed backfill round-trip for T9.
