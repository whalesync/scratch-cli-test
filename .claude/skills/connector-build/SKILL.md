...
- **`server/src/remote-service/connectors/library/<connector>/PLAN.md`** — *this* connector's **active plans**: atomic, concise plan items for substantial changes, each marked `APPROVED` or `FOR_REVIEW`, awaiting or cleared for execution. One per connector. (What we're about to change — see [the plan flow](#the-coverage-doc--statemd).)
...
5. A **Milestones** table near the top — a 9-row "where are we" tracker so anyone can see the connector's progress at a glance, each row ✅/🔄/⬜: **(1) account ready** (registered/logged into the web app) → **(2) connected** (health OK) → **(3) first fetch** (≥1 record pulled) → **(4) all entities seeded & fetched** → **(5) full write CRUD** (create+edit+delete pushed) → **(6) foreign keys tested** (CLI move parent→parent) → **(7) edge cases & quirks tested** → **(8) view(s) built** (default view with fields grouped logically — see [Building views](#building-views--group-by-existing-mechanics-never-invent)) → **(9) OAuth** (final/pre-release — create the OAuth client with the user via the browser, or document what it requires; see [OAuth — the final milestone](#oauth--the-final-milestone)).
   ...
   **Substantial changes get a plan — `PLAN.md`.** Anything beyond a small fix shouldn't be acted on straight from the checklist — give it a corresponding entry in **`PLAN.md`**, a per-connector doc in the connector folder alongside STATE.md and LOG.md (`server/src/remote-service/connectors/library/<connector>/PLAN.md`). As you test you flip coverage cells to ✅ and surface new TODOs, and each substantial TODO becomes an **atomic, concise plan item**; a human then reviews and edits the plan and hands you instructions to execute it — often adding more items along the way. **Every plan item carries a status — `APPROVED` or `FOR_REVIEW`.** Work freely on `APPROVED` items, and freely *add* `FOR_REVIEW` items, but leave a `FOR_REVIEW` item for a human to approve before you execute it. **Small fixes are the exception — apply them immediately, with no plan entry and no approval.** Rather than writing a plan inline, **define the problem in a clear, self-contained prompt and spin a subagent to draft that one plan item** into PLAN.md, then move on to other work while it does.

   **What makes a plan item good is the *example*, not the prose — treat this as the single most important rule of a plan.** Every plan item must carry a **concise, concrete example** of the change: a **minimal** sample — a trimmed record, the key API call + response, the CLI command — using **real** values but kept as **short as it can be while still cleanly demonstrating the situation** (show only the fields that matter; drop the noise). Aim for an example a human can read and grasp in seconds, not a verbatim dump. A clear example is what makes a plan item good; the explanation still matters, but the example is what makes the item reviewable and actionable, and it should carry more of the weight than the description. Whenever the change has a visible effect, write the example as a **before/after**: **"Here is what happens now: …"** (the current record / call / output, trimmed to what matters) then **"Here is what will happen after the fix: …"** (the same, post-change). Aim for a reviewer to be able to understand and approve the item from the example alone.

   **`PLAN.md` holds only *active* plans — implemented ones move to `ARCHIVE.md`.** When a plan item ships, **append it to `ARCHIVE.md` (a sibling doc) and delete it from PLAN.md**, so PLAN.md stays short — just the live, concise plans — and never balloons your context. ARCHIVE.md is write-mostly history: **don't read it in the normal loop**; open it only when you need to revisit how a past change was made (e.g. one that later turns out problematic).
