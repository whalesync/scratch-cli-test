...
**Three run options — try them IN THIS ORDER, use the first that works:**
1. **gstack headless** (`$B connect`) — isolated, no window. **Prefer this** for unattended/overnight and for parallel agents (each gets its own browser, zero collisions).
2. **gstack headed** (`$B connect --headed`, `--headed` on *every* command) — same engine, visible window. Use when a human must watch or pass a gate, or if headless is acting up.
