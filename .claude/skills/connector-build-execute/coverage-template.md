...
/connector-build-execute. Legend: ✅ verified against the service · ⬜ not yet ·
...
| 10 | **Integration test** (live-API spec in `server/test/integration/<svc>-connector.spec.ts` covering schemas / pull / publish / errors; wired into post-deploy CI) | ⬜ | **Required.** Details in the [Integration tests](#integration-tests) section. Can be built any time after Milestone 5. **Optional but recommended:** an idempotent `scripts/bootstrap-<svc>-test-data.ts` seed script (see that section). |
...
## Integration tests
Automated **live-API** coverage in `server/test/integration/<service>-connector.spec.ts`, and whether it runs in the **post-deploy CI job** (`gitlab-ci/stages/06-environment-tests.yml` → `environment tests for test env post-deploy`, rule `on_merge_to_master`). This is the CI regression backstop — distinct from the manual coverage matrices above (which record what a human verified through the CLI/UI). Cross-connector view: [`connector-build/existing-connectors.md` → Connector summary table](/connector-build/existing-connectors.md) (columns **IT 📄** = a spec exists, **IT ✅** = it runs in the pipeline). Mechanics of the harness (in-process, hits the third-party API directly, `describeIfKey` self-skip, `maxWorkers:1`): [`docs/plans/integration-tests/dev-10304-automated-connector-tests/dev-10304-automated-connector-tests.md`](/docs/plans/integration-tests/dev-10304-automated-connector-tests/dev-10304-automated-connector-tests.md).

- **Live spec:** `<server/test/integration/<service>-connector.spec.ts | none yet>` — 📄 ✅ exists / ❌ none.
- **Runs in CI pipeline:** `<✅ | ❌>`. Wired via `<ENV_VAR(s)>` mapped from GitLab CI/CD masked variable(s) `INTEGRATION_TEST_<SVC>_*`. Each suite self-skips (`describeIfKey`) when its key is unset, so an unwired key just skips — only **Notion** is wired today.
- **Credentials / env vars:** `<VARS>` — local in `server/.env.integration` (template `.env.integration.example`); CI in GitLab → Settings → CI/CD → Variables.
- **Capabilities covered:** schemas `<✅/❌>` · pull `<✅/❌>` · publish (CRUD) `<✅/❌>` · error handling `<✅/❌>`.
- **State model:** `<self-provisioning (seed → round-trip → cleanup, state-agnostic) | depends on pre-seeded fixtures — list them, e.g. a named DB / specific base/list id / bootstrap script (drift-prone)>`.
- **Seed script (optional but recommended):** `<scripts/bootstrap-<svc>-test-data.ts | none>` — an **idempotent** script that provisions the **long-lived** data the suite needs on a fresh account (custom fields/attributes, lists, reference/lookup records — things that can't be created per-run or are expensive to). Lets a brand-new dedicated test account be stood up in one command, and re-running heals a wiped account. Reference: `scripts/bootstrap-attio-test-data.ts`.
- **Notes:** `<fake-backed (test-api-fakes/<svc>) | read-only suite | drift risk + DEV link | special setup | none>`.
